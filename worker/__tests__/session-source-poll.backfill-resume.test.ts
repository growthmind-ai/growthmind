...
//
// CR-3 / CR-4 ADDENDUM (this file's tests below "CR-1 fix" above):
//
// Code review found the CR-1 fix above closed the LITERAL bug it targeted
// (no cursor written at all) but left a narrower, still-live gap in the same
// function: when a walk's `contiguous` is `false` AND its `resumeBefore` is
// `null` — the adapter's CR-11 case, "this specific walk got zero of the
// shared page budget because an earlier walk in the same pull already spent
// it all reaching `exhausted`" — `applyCursors` wrote NOTHING (the old
// guard was `!result.contiguous && result.resumeBefore !== null`). The
// connection then re-read the SAME stale `backfill_before` next tick,
// reproduced the identical request sequence, and could never see anything
// older than that fixed point — a livelock, not merely a slow catch-up.
// Fixed by writing `result.resumeBefore` (including `null`) whenever
// `!result.contiguous`, in both `applyCursors` here and the mirror branch in
// `packages/db/src/services/connections.service.ts`'s `performFirstPull`.
//
// A SEPARATE, HARDER QUESTION surfaced while building the "eventually
// reaches contiguity" test CR-4 asked for: with `MAX_PAGES_PER_PASS` (here)
// and `MAX_PAGES_PER_RUN` (the adapter) both `25`, and the forward pass
// (`pass 2`) ALWAYS restarting from the true newest event whenever
// `watermark_at` is still `null` (never partially resuming), reaching
// `contiguous` requires pass 2 to walk its ENTIRE backlog inside the budget
// LEFT OVER after the backward resume pass — which is provably impossible,
// for any tick count, once that backlog exceeds one pull's page cap
// (`MAX_PAGES_PER_RUN × PAGE_LIMIT` = 5,000 events): the backward pass's
// own cost only grows as the forward frontier recedes, so the two-cursor
// state cycles rather than converges. Driving `runSessionSourcePoll` twice
// in a row therefore cannot demonstrate "the resumed walk eventually
// reaches contiguity" for a backlog that large — no code change inside this
// file's scope (worker/, or packages/db's connection service and
// repository) can make it converge, because the forward pass restarting
// from scratch is the adapter's (`packages/adapters`) own contract. The
// convergence test below instead exercises the achievable, and far more
// common, case this two-cursor design is actually built for: a connect-time
// first pull's own tiny `FIRST_PULL_MAX_PAGES = 1` cap, completed by the
// very next scheduled poll's much larger budget — proving the resume
// cursor CR-1/CR-3 now reliably records is correctly picked up and DOES
// reach contiguity there. The un-converging >5,000-event case is reported
...
/**
 * An upstream backed by a FIXED, finite list of event instants (newest
 * first) rather than an infinite generator or a global call counter. Reads
 * are computed from the request's OWN `before`/`after` query parameters —
 * exactly like a real server — so this fake behaves correctly no matter
 * which walk (a fresh forward pass or a resumed backward one) issues the
 * request, and no matter how many prior calls this `FakePostHog` instance
 * has already served. That independence from call order is what makes it
 * safe to drive across two SEPARATE `runSessionSourcePoll` invocations
 * (or a `connect()` call followed by one) without the walk that resumes
 * ever seeing the wrong slice.
 */
function positionalBacklog(params: {
  ...
  return (request: FakeEventsRequest): FakeEventsPage => {
    ...
    // One event per page, matching this file's other fixtures — it is the
    // PAGE COUNT the page cap governs, not the event count per page.
...
test("CR-3: a walk that stops with contiguous:false AND resumeBefore:null (the adapter's CR-11 zero-budget case) still clears the stale cursor instead of livelocking forever", async () => {
  // 26 events, newest (index 0) to oldest (index 25), one page apart. The
  // connection is seeded ALREADY mid-backfill — `backfillBefore` points
  // just past index 0 — so the very first `runSessionSourcePoll` call below
  // plays the role of "a later tick", after some earlier run already
  // established that cursor.
  const eventTimes = Array.from(
    { length: 26 },
    (_, i) => new Date(NOW.getTime() - (i + 1) * 60_000),
  );
  const env = testServerEnv();
  // Fixed rather than auto-generated, so the seeded `backfillBefore` below
  // can be built from the SAME `sourceProjectId` the walk will actually
  // request against.
  ...
  // TICK A. The resume walk needs EXACTLY 25 fetches (index 1 through 25)
  // to reach the true end — using the WHOLE shared page budget for this
  // pull, so the forward pass that would normally run afterward gets zero
  // of it and hits the adapter's own page cap without ever sending a
  // request. That specific combination — `contiguous: false`,
  // `resumeBefore: null` — is exactly the case the OLD `applyCursors` guard
  // (`!result.contiguous && result.resumeBefore !== null`) skipped writing
  // anything for.
  await runSessionSourcePoll(createPollDeps({ db, fetch: posthog.fetch, clock }));
  ...
  // THE FIX: the stale seeded cursor is CLEARED, not left in place. Before
  // the fix this stayed equal to the original seeded value forever.
  ...
  // TICK B. Because the cursor was cleared, this tick's resume pass is
  // SKIPPED and the forward pass runs fresh with the FULL page budget —
  // fetching a genuinely different slice (index 0 through 24) rather than
  // reproducing tick A's exact request sequence.
  clock.advance(60_000);
  await runSessionSourcePoll(createPollDeps({ db, fetch: posthog.fetch, clock }));

  const afterTickB = await connectionRow(seeded.connectionId);
  // Real, new progress: the cursor moved again, to a position that is
  // neither the original seed nor `null`.
  expect(afterTickB?.backfillBefore).not.toBeNull();
  expect(afterTickB?.backfillBefore).not.toBe(originalSeedCursor);

  // The proof that matters: WITHOUT the fix, tick B would have replayed the
  // identical index-1-through-25 request forever, and index 0 — the single
  // NEWEST event, the one a customer looks at first — would never have been
  // fetched by any tick, ever. With the fix, the union of both ticks' walks
  // covers the whole 26-event backlog.
  const events = await eventsFor(seeded.projectId);
  expect(events.length).toBe(eventTimes.length);
});

test("CR-3/CR-4: a connect-time first pull that hits its own tiny page cap is completed by the very next scheduled poll — the watermark actually moves", async () => {
  // Deliberately only 3 events: `FIRST_PULL_MAX_PAGES = 1` (connect-time)
  // cannot cover more than one page, so this still forces a page-capped
  // first pull, but the WORKER's much larger `MAX_PAGES_PER_PASS = 25`
  // budget can easily walk the whole remaining backlog in its very next
  // tick — the scenario CR-3's own language ("any project with >200
  // events") describes, and the one this two-cursor design can provably
  // resolve in two pulls (see this file's header comment for why a
  // worker-poll-only "twice" cannot, for any backlog exceeding one pull's
  // own page cap).
  ...
  // PULL TWO OF TWO — the very next scheduled poll, through the REAL
  // worker entry point (`runSessionSourcePoll`), exactly as `taskList`
  // invokes it.
  clock.advance(60_000);
  await runSessionSourcePoll(createPollDeps({ db, fetch: posthog.fetch, clock }));
