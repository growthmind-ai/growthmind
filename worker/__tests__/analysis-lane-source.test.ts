// O-012's end-to-end proof: PERSISTED events through to a `CandidateFinding` —
// the first test in this repository that drives the whole producing pipeline
// against a real database. Rows go into `sessions` and `events` through the
// schema barrel against a real `createTestDb()` PGlite instance; the REAL
// corpus service reads them, the REAL detectors propose, the REAL gate
// concludes, and the assertion at the end is on the lane the REAL producer
// hands the tick's port.
//
// House rules: fixture time is frozen constants (no `Date.now()`); the
// producer receives `now` as a parameter, which is the property that makes the
// replay assertion at the bottom meaningful at all.
...
/** Persists one kept session walking `paths`, one event per step — the same
 * row shapes `persistPullResult` writes, seeded directly so this file tests
 * the producing read, not the poll. */
...
describe("createAnalysisLaneSource — persisted events to a CandidateFinding (O-012 e2e)", () => {
  test("drives persisted sessions and events through corpus, detectors and gate into one lane's CandidateFinding", async () => {
    ...
    // The funnel shape the assembler suite proved pure, persisted for real:
    // 3 strugglers who leave and return three times (and continue), 12 who
    // drop at the origin, 15 who convert — 30 kept in-window sessions, a 40%
    // drop at the inclusive boundary, both struggle magnitudes at their
    // minimums.
    ...
    // The lane's tenant scope comes from the seeded rows themselves (D7).
    expect(lane.organizationId).toBe(workspace.organizationId);
    expect(lane.organizationName).toBe(workspace.organizationName);
    // The corpus's own denominator: 30 kept in-window sessions — the
    // before-window session never reached the read.
    ...
    // Re-parse through the contract: what the producer yields is exactly what
    // every downstream consumer compiles against, brand and refinements
    // included.
    ...
    // The count carries the corpus's denominator — the same 30 the lane
    // states, so a rendered "12 of 30" can never disagree with the lane.
  ...
  test("a project whose connection is inactive is not analysed (D2 — the selection is explicit)", async () => {
  ...
  test("a connected project with no sessions yields a lane with sessionsConsidered 0, never a crash (D5)", async () => {
  ...
  test("a lane whose sessions all fail the gate yields candidates [] with sessions considered, and logs the rejection", async () => {
    ...
    // struggled, so `confusing`'s only proof is absent and the gate drops the
    // proposal (FR-13B: never downgraded to the class that blames the user).
