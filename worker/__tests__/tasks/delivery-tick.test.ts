// The delivery lane's composition root, driven end to end (O-007).
//
// Every test calls `runDeliveryTick` — the REAL handler, the same plain
// function `taskList` invokes — against FAKES THAT HOLD REAL STATE: the ledger
// fake stores rows and answers the next claim from them, so "claim, post, then
// tick again" is a real sequence rather than a script of expected calls. A mock
// asserting `markPosted` was called would pass just as happily against a
// handler that never wrote anything a second tick could read (D11).
//
// The ledger fake is typed as `DeliveriesRepo` — the SHIPPED interface, not a
// hand-written look-alike — so it cannot drift into agreeing with a repository
// that no longer exists. The real SQL behind that interface is proven against a
// real database in `packages/db/__tests__/repositories/deliveries.repo.test.ts`;
// what this suite owns is the ORDER the handler calls it in.
//
...
/** A real 64-char lowercase hex digest, built through the real constructor —
 * the branded type has no other producer. */
...
/** A count built by the ONE constructor — the brand cannot be faked, and the
 * renderer refuses anything that is not one. */
...
/** The renderer's deliver arm. `context` is the model-written prose — the one
 * place in a rendered message that could carry text nobody vetted, which is
 * exactly what the residual gate exists for. */
...
interface FakeLedger {
  ...
  /** Makes every read/write for one project throw — a repository fault, the
   * class of failure the per-lane isolation exists for. */
...
function createFakeLedger(): FakeLedger {
  ...
  return {
    ...
    repoFor: (ctx) => ({
      // The atomic claim, with the real repository's rule: a row that exists is
      // only re-claimable when it is `failed`.
      ...
      markFailed(input: MarkFailedInput): Promise<DeliveryRecord | null> {
        const key = keyOf(ctx.organizationId, input.findingId, input.channelId);
        const existing = stored.get(key);
        // The real repository's `status <> 'posted'` guard: a late failure
        // signal must never rewrite a posted row into a re-claimable one.
...
/** The port, faked. Default behaviour is the happy one; a test that cares
 * passes its own answer — including one that THROWS, which the port forbids and
 * the handler must survive anyway. */
...
test("a second tick over an already-posted finding posts nothing and leaves the row untouched", async () => {
  // The D4 guarantee across two real ticks: the first tick's persisted row is
  // what stops the second, not an in-memory flag.
  const scene = harness({ lanes: [lane()] });
...
test("a post another worker already owns is never attempted", async () => {
  ...
  // The lane must still reach the claim to be a real test of it, so the open
  // finding IS the one being delivered — the scheduler has nothing to withhold,
  // and the claim is the only thing that can refuse.
  const scene = harness({
...
test("a finding still pending for the project withholds a different finding from the same lane", async () => {
  // The wire from `listPendingForProject` into `decideDelivery`'s
  // `openFindingIds` — a value the handler must READ rather than be told (D11).
  const ledger = createFakeLedger();
...
test("a poster that refuses records the delivery failed and leaves the finding deliverable", async () => {
  ...
  // "Leaves the finding deliverable" is a claim about the NEXT tick, so it is
  // asserted by taking one: a failed row is re-claimable.
...
test("a poster that throws still reaches a terminal failed state and never escapes the tick", async () => {
  ...
  const row = scene.ledger.rowFor("finding-1");
  expect(row?.status).toBe("failed");
  // A row left `pending` is what jams the lane forever — the one state this
  // path must never produce.
  expect(row?.status).not.toBe("pending");
  // The thrown error's own text never reaches the customer-facing column: it can
  // carry vendor detail, ids and stack text. It goes to the log instead.
...
test("no lane can leave a delivery stuck pending, whatever the poster does", async () => {
  // The D8 invariant stated as one assertion over every failure shape at once.
...
test("generated text carrying personal data is never posted and the recorded reason quotes none of it", async () => {
  // No cohort noun anywhere in this sentence, deliberately: the renderer drops
  // model prose that calls sessions people, and prose it drops is prose the
  // residual gate never gets to refuse. This fixture reaches the gate.
  ...
  // sentence it sat in. Recording the match would copy the personal data into
  // the very place we refused to send it.
...
test("the residual gate scans the exact text the poster is handed", async () => {
  // The D11 proof: a gate that clears one string while a different string is
  // posted is a gate that does nothing. `textPostedFor` is the ONE derivation,
  // and it is derived from the request object itself.
  const scene = harness({ lanes: [lane()] });
  await scene.run();
  ...
  // Every block's prose is inside the scanned string — a block the gate cannot
  // see is prose that reaches Slack uncleared.
...
test("a message the residual gate cannot read is refused rather than cleared", async () => {
  // Fail direction, made explicit: `null` means "cannot be cleared", never
  // "clean". A block graph JSON cannot represent is the realistic shape.
...
test("nothing_today is logged with its reason, posts nothing, and writes no delivery row", async () => {
  ...
  expect(summary.nothingToday).toBe(1);
  expect(scene.posted.length).toBe(0);
  // The `deliveries` table has no row shape for a nothing-today — no
  // `finding_id` — so there must be nothing here at all.
...
test("a lane that decides nothing_today on every tick still never posts", async () => {
  // What stops a nothing-today being sent repeatedly is that it is never sent
  // at all — there is no persisted key on which "did we already say this?"
  // could be asked (see the handler's header, TODO O-008).
  const scene = harness({ lanes: [lane({ candidates: [] })] });
...
test("the delivery task name parses as a Graphile Worker crontab identifier", () => {
  // The separator must be a COLON. A dot passes every unit test and then
  // crashes the worker on boot with "Invalid command specification in line N of
  // crontab" — a failure only a running container ever sees.
