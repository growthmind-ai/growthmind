// O-003 §9 items 115–120 — THE FR-21 END-TO-END WIRE PROOF (D11).
//
// The single most important test file in the sprint. The previous sprint
// shipped exactly this bug: a value computed by one surface with zero
// production consumers, where a producer test passed, a consumer test passed,
// and the wire between them was never connected.
//
// So every test below drives the REAL CONSUMER ENTRY POINT — the worker task
// handler `runSessionSourcePoll`, invoked exactly as `taskList` invokes it:
// a plain call with the deps the queue closure assembles, and no payload —
// against a FAKED PostHog HTTP layer and a REAL `createTestDb()` database.
// Then it asserts the effect actually fired, at both ends of the five-link
// chain:
//
//   credentials → SessionSource pull → exclusion evaluation → persistence
//                                                              → counter
//
// Nothing here reaches past the handler. There is no direct call to the
// adapter, no direct call to `persistPullResult`, and no hand-built pull
// result. The credential is encrypted with the REAL `encryptSecret` against
// the REAL key the handler's own environment resolves, so link one is proven
// too rather than stubbed around. The counter is read through the REAL
// `createEventsCounterService`, so link five is a live read of persisted rows
// rather than a returned summary the handler happened to compute.
//
// FIXTURE SEED PREFIX: `e2e-`. Every org name, user email, and project name
// carries it plus a uuid, so this suite can never collide with another lane's
// fixtures on `user_email_unique`.
//
// WAVE 0: the chain is typed stubs end to end. Every test here MUST fail on a
// stub's "not implemented", never on a compile error or a fixture collision.
 ...
 * `resolveCredentialKey` derives from the same `ServerEnv` the handler is
 * given — a hand-written placeholder would only ever exercise the F-11
 * fail-closed branch and would prove nothing about link one of the chain.
 */
...
/** THE REAL ENTRY POINT. Every test goes through this one call — the same
 * plain function the queue closure in ../src/index.ts invokes, with the same
 * deps and no payload. */
...
// `@growthmind/db`, not on the ORM directly.
...
// The upstream page every test builds on: one ordinary visitor with an
// external email (kept) and one headless session (set aside).
...
test("credentials → pull → exclusion → persistence → counter: the handler persists sessions and events with the expected exclusion_reason stamps AND the counter returns the matching kept/set-aside breakdown", async () => {
  ...
  // Link five: the counter, read through the real service against the rows
  // the handler just wrote. This is the assertion the previous sprint's bug
  // would have failed — a computed value with no live consumer.
...
// ---------------------------------------------------------------------------
// Item 116 — idempotency THROUGH the real entry point (FR-6 / D4)
// ---------------------------------------------------------------------------
...
test("running the handler twice against the same upstream page yields one row per event and one session", async () => {
  ...
  await invokeTheHandler({ posthog });
  // The claim moved `next_poll_at` forward, so the second tick is a later
  // moment — exactly as the cron would arrive.
  ...
  // A replayed task produces the same end state: the unique index on
  // (project_id, source_event_id) absorbs the re-query, and the session upsert
...
// ---------------------------------------------------------------------------
// Item 117 — out-of-order arrival, recovered by the overlap re-query (FR-5)
// ---------------------------------------------------------------------------
...
test("an event whose declared timestamp predates the watermark is persisted exactly once by the overlap re-query", async () => {
  ...
  // The decisive shape the probe pinned: `timestamp` is the CLIENT-DECLARED
  // event time, so a late-flushing SDK buffer lands an event BEHIND the
  // watermark. `after = watermark − overlap` is the only thing that ever sees
  // it, and no ingestion-time field exists to poll on instead.
  ...
  // The request reached back BEFORE the watermark — otherwise this event
  ...
  // Run again over the same page: the overlap re-queries it every time, and
  // the dedup key absorbs it. Exactly one row, forever.
  await invokeTheHandler({ posthog, now: new Date(NOW.getTime() + 5 * 60_000) });
...
// ---------------------------------------------------------------------------
// Item 118 — the exclusion predicates, end to end (FR-12 / FR-13)
// ---------------------------------------------------------------------------
...
test("a headless-UA session and an internal-domain session are set aside, and an ordinary session with an external email is kept — end to end", async () => {
  ...
  // The stamp records its own provenance, so it stays reproducible from
  // persisted data with no upstream access.
...
// ---------------------------------------------------------------------------
// Item 119 — per-item degradation, surfaced in the counter (FR-3 / D5)
// ---------------------------------------------------------------------------
  ...
  const posthog = createFakePostHog({
    events: (): FakeEventsPage => ({
      results: [
        ...
        // One weird item must never stall a connection forever. It is skipped,
        // COUNTED, and surfaced — never silently discarded.
...
// ---------------------------------------------------------------------------
// Item 120 — the teammate sees what the owner sees (FR-20 / D1 / P-4)
// ---------------------------------------------------------------------------
...
test("a teammate's tenant context reads the same counter values the connecting owner sees", async () => {
  ...
  // An org-scoped effect written by a scheduled job must be readable by every
  // member of that org — not only by whoever attached the connection. The
  // teammate is a real non-owner `member` row, so this is asserted rather than
  // assumed.
...
// A guard on the proof itself: `nextCursorUrl` builds the absolute, verbatim
// cursor the server would emit, so a walk in any test above can only FOLLOW it
