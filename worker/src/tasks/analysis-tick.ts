...
// composes the tick, `../analysis-lane-source.ts` implements its port, and the suite
// drives `runAnalysisTick`. Every one of them names this file, and none of them should
// have to learn the lane's internal layout to do it.
export type {
  AnalysisLane,
  AnalysisLaneSource,
  AnalysisLogger,
 ...
 * One project's turn.
 *
 * Returns an outcome on every path. The run is opened first and closed on every path
 * out of a successful open, including the one where a candidate's persistence fails,
 * which ends the lane rather than continuing against a store that has stopped
 * answering.
 */
