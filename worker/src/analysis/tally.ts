...
//
// Every function here is PURE in the sense that matters: no I/O, no clock, no
// logger, no repository. `applyAttribution` mutates the tally it is handed and
// returns nothing, which is a deliberate choice rather than an oversight — the
// walk is a loop over candidates and a fold that allocated a fresh tally per
// candidate would read worse for no gain. The mutation is confined to a value
// the caller owns and never escapes this module's callers.
//
// Kept apart from the ladder because it answers a different question. `plan.ts`
// decides what happens to ONE candidate; this decides what the RUN ROW says
// about all of them, and every exit path — ordinary, refused or thrown — closes
// from one accumulated set of facts rather than from a value some branch forgot
// to set.
 ...
 * processed, so every exit path closes from one accumulated set of facts. */
...
/**
 * Adds a reported token count to a running total, preserving the
 * not-reported/zero distinction.
 *
 * `undefined` in leaves the total exactly as it was — including `null`. Only a
 * genuinely reported number promotes a `null` total to `0 + reported`.
 */
...
export function applyAttribution(tally: RunTally, attribution: CallAttribution): void {
  ...
  // (AD-5). That holds because `CallAttribution`'s `attempted: true` arm cannot
  // carry a null id: this aggregate is only as true as its inputs, and a run
  // whose every call threw used to close with `modelCallsAttempted > 0` beside
  // a null model id.
 ...
 *
 * The two zeros stay distinct: "we have not looked yet" and "we looked and your
 * product was quiet" are different answers, and only the lane source knows
 * which applies.
 *
