// The T1 event-vocabulary probe's PURE core (O-004 FR-21, ADD §6.3).
//
// PURE: no clock, no I/O, no module-scope state read inside a judgement. Every
// function here is unit-tested in `__tests__/t1-vocabulary.test.ts`, which is
// the specification — the impure shell that feeds it lives in
// `../t1-event-vocabulary-probe.ts`.
//
// -------------------------------------------------------------------------
// WHY THIS MODULE EXISTS, AND THE ONE RULE IT ENCODES
// -------------------------------------------------------------------------
// ADD §2 could not pin A-2…A-5 (`$rageclick`, `$dead_click`/`$dead_swipe`,
// `$autocapture`, `$pageview`) because the project it read held 220 events,
// ALL of them written by this repo's own spikes, and ZERO originating from a
// browser SDK. Zero observations of a browser event in that corpus measures
// OUR OWN SYNTHETIC WRITES — not the customer's client config. Calling that
// `PINNED — absent` would have shipped `rage_click`/`dead_click` as "does not
...
//     detector is never built on it (ESC-2, D-17).
//
// Fail direction (D10): every classification here fails TOWARD
// `FAILED-TO-PIN`. An unrecognised `$lib` is not counted as browser traffic,
// an unparseable row is dropped from the denominator, and both push the sample
// further from "representative" — i.e. away from an absence claim we cannot
// support. Over-claiming absence is the failure this module exists to prevent;
// over-claiming inconclusiveness merely costs a re-run.
...
/**
 * One event, reduced to exactly what the vocabulary question needs. Built from
 * the events list API's raw items, which are `unknown` by construction: a live
 * project holds every payload shape ever written to it (D5), so nothing here
 ...
 *
 * `denominator` is the whole point: a count without one is a number nobody can
 * argue with, and §2's entire A-2…A-5 conclusion turned on "0 / 220" being
 * readable as a ratio rather than as a zero. `counts` never omits a name it
 * observed, and never invents one it did not.
 */
...
/**
 * The rules the representativeness judgement runs under, passed as a
 * PARAMETER to every function that consults them — never read from module
 * scope inside a judgement. A gate whose thresholds are reachable only through
 * its argument is a gate a test can move; one that reads a constant internally
 * is a gate a test can only agree with.
 */
export interface RepresentativenessRules {
  readonly version: number;
  /**
   * `$lib` values that count as a REAL browser client. An ALLOW-LIST, never a
   * deny-list: an unrecognised `$lib` is not browser traffic, which withholds
   * an absence claim rather than manufacturing one (D10 fail direction).
   */
  readonly browserOriginLibs: readonly string[];
  /**
   * The minimum browser-originated events an absence claim needs behind it.
   * ADD §2: 220 synthetic events were "far under the ~50-real-browser-event
   * bar that would make an absence claim meaningful".
   */
...
/** ADD §2's stated bar for a meaningful absence claim. */
...
/**
 * Why a sample cannot carry an absence claim. Two reasons, both named — the
 * first is §2's exact situation, the second is the near-miss beside it (a
 * project with SOME real browser traffic, but not enough of it).
 */
...
/**
 * A sample that COULD have contained the events under test. The only type an
 * absent verdict accepts, and `judgeRepresentativeness` is its only producer.
 */
...
/**
 * One vocabulary claim under test. `eventNames` is the set of literals whose
 * presence would settle the row — more than one where the row asks about a
 * family (A-3 asks about `$dead_click` AND `$dead_swipe`).
 */
export interface VocabularyRow {
  /** ADD §2's row id, so a re-run's output lines up against the original table. */
  readonly id: string;
...
/**
 * ADD §2's sample-observable rows. A-6 and A-7 are deliberately absent: both
 * are STATIC source verifications about what the adapter reads, settled by
 * reading `constants.ts`/`parse.ts`, and no volume of live traffic can move
 * either. A probe that pretended to re-test them would be reporting a verdict
 * it never measured.
 */
...
/** The three verdicts a row can carry. The em dashes are ADD §2's, verbatim. */
 ...
 *     supplied in its place. This is ESC-2's lesson made unrepresentable
...
/**
 * What a `null` `$lib` is REPORTED as in `SampleBasis.observedLibs`. An event
 * with no `$lib` still counts in the denominator and still shows up in the
 * observed-lib list — omitting it would hide the very thing ruling 11 asks a
 * human to confirm before trusting an absence claim.
 */
 ...
 * deterministic ordering in this sprint reads the same way. `localeCompare`
 * would make the output depend on the runner's locale.
 */
 ...
 *
 * An item without a string `event` name is DROPPED, not defaulted: it cannot
 * answer a vocabulary question, and dropping it shrinks the denominator, which
 * pushes the sample away from "representative" — the safe direction. A missing
 * or unparseable `$lib`/`timestamp` becomes `null`; only the name is required.
 */
export function toObservedEvents(rawItems: readonly unknown[]): readonly ObservedEvent[] {
  ...
  for (const item of rawItems) {
    ...
    const { event } = item;
    // An empty name is dropped for the same reason a missing one is: it names
    // no vocabulary, and dropping shrinks the denominator (the safe direction).
 ...
 *
 * An empty sample returns `{ denominator: 0, counts: [], window: null }` —
 * never a throw, never a fabricated zero-count row for a name nobody observed.
 */
export function buildEventNameHistogram(sample: readonly ObservedEvent[]): EventNameHistogram {
  ...
  for (const event of sample) {
    ...
    // The window spans the earliest and latest USABLE timestamps — scanned, not
    // read off the first and last array positions, which the API's ordering
    // does not guarantee and a null timestamp would break outright.
 ...
 *
 * A `null` lib is NOT browser-originated. Neither is an unrecognised one —
 * including a future posthog-js `$lib` value this list has not learned yet.
 * That miss costs a `FAILED-TO-PIN` and a line in the report saying which libs
 * were observed; the opposite fail direction would let our own synthetic
 * `gm-shape-probe` writes underwrite an absence claim, which is precisely
 * ESC-2 (D10).
 */
 ...
 *
 * Otherwise `not_representative`, with `no_browser_originated_events` when the
 * count is zero (§2's exact situation) and
 * `below_minimum_browser_denominator` when it is non-zero but under the bar.
 * `basis` is populated identically either way, so the report can state the
 * numbers whichever branch it took.
 */
export function judgeRepresentativeness(
  ...
  // Populated identically on both branches: the report states the numbers
  // whichever way the judgement went, so a FAILED-TO-PIN line says how far
  // short the sample fell rather than merely that it did.
 ...
 *
 * `denominator` is `histogram.denominator` on every arm, so no verdict line can
 * be read without the number it was measured against.
 */
export function verdictForRow(
  ...
  // Only literals the histogram actually carries — a verdict never reports a
  // name it did not see, so a family row (A-3) lists exactly the half observed.
  ...
  // Absence is the gated arm. TypeScript enforces it too — the basis below is
  // narrowed to `RepresentativeSample`, which is the whole of ESC-2's lesson.
...
/**
 * Composes the whole judgement: histogram, then the gate ONCE for the sample,
 * then one verdict per row against that single judgement.
 *
 * The gate is evaluated once and shared deliberately — representativeness is a
 * property of the SAMPLE, not of a row, and re-deriving it per row is how the
 * two drift apart.
 */
...
/**
 * One printable line per verdict, in ADD §2's reporting shape:
 * `A-2  FAILED-TO-PIN  $rageclick  0 / 220 — no browser-originated events…`
 *
 * PURE and therefore testable, but NOT trusted to be safe on its own: the
 * entrypoint pipes every line through `lib/redact.ts` before it reaches stdout
 * or disk, because an event NAME is customer-authored text and can carry an
 * identifier we never anticipated.
 */
