 ...
 */
/**
 * The published dev-only credential-encryption key (O-003 D-1). Base64 of 32
 * bytes, so it is a structurally valid AES-256 key and `docker compose up`
 * from a clean clone needs no .env — the same self-host promise
 * BETTER_AUTH_SECRET's dev default keeps.
 *
 * It is published, so it is worthless as a secret. `resolveCredentialKey`
 * (src/crypto/credential-key.ts) refuses it in production at the *encryption
 * call site* — a second check that GROWTHMIND_ALLOW_INSECURE_DEFAULTS cannot
 * open, because the blast radius of this key is a third party's PostHog
 * account rather than this deployment's own sessions.
 */
...
export const serverEnvSchema = z.object({
  ...
  /**
   * O-003 D-14: validated-if-present, never read by the adapter. Customer
   * credentials come exclusively from `project_connections` — reading a
   * global env key would be a single-tenant design in a multi-tenant
   * product. These exist so local tooling and scripts/spikes/* get the same
   * validation the app gets, and so a self-hoster with no PostHog boots
   * cleanly (OQ-4 graceful absence).
   */
