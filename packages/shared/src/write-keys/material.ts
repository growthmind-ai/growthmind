 ...
 * presented key (correct prefix, correct length/charset). Never throws on
 * malformed input.
 */
...
/**
 * Deterministic SHA-256 hex digest of the raw key material (node:crypto).
 * The material is high-entropy random (256-bit), so a fast deterministic
 * hash is correct here — this is a lookup key, not a password hash.
 */
