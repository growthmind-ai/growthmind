// Wave 0 (RED) — O-009 `mcp-read-credential`, ADD §8 "Unit tests (pure
// functions)", all 8 rows.
//
// Subject: `packages/shared/src/api-keys/material.ts` (ADD D-2) and
// `packages/shared/src/api-keys/types.ts`. Neither exists yet, so this suite
// is red at module resolution until Wave 1 fills them in — that IS the stated
// reason, not an accident of path.
//
// The cross-lane loop this file owns one half of (ADD D-2): `packages/shared`
// may not import `packages/db`, so nothing here can call the real `mint()`.
// This file therefore constructs material the way `mint()` will
// (`API_KEY_PREFIX + randomBytes(32).toString("base64url")`, mirroring
// `generateRawKeyMaterial` at write-keys.repo.ts:45-47) and asserts the format
// accepts exactly that; `packages/db/__tests__/repositories/api-keys.repo.test.ts`
// asserts `isApiKeyFormat(minted.raw)` on a genuinely minted key. Neither half
// alone proves the round trip.
//
// D9/D10: `gmak_` and `gmwk_` differ at index 2 only. Both confusion
// directions are named rows below, and every one of them carries a
// non-vacuity control — a fixture that is genuinely well-formed for its own
// family — so a row can never pass because the sample was junk.
//
...
/** Exactly what `mint()` will generate — the same expression
 * `generateRawKeyMaterial` uses, with this family's prefix. */
...
describe("isApiKeyFormat", () => {
  ...
  test("should refuse empty, whitespace, 42-char, 44-char, wrong-charset and 10 000-char input without throwing", () => {
    ...
    for (const value of refused) {
      // Fail-closed (D10) and never by exception — a credential gate that
      // throws on hostile input is a 500, not a refusal.
      expect(() => isApiKeyFormat(value)).not.toThrow();
      expect(isApiKeyFormat(value)).toBe(false);
    }

    // Control: the same generator at the correct length IS accepted, so the
    // rejections above are about the input, not a permanently-false gate.
  ...
  test("should refuse a leading space, because the presenter does not trim", () => {
    ...
    // trim, deliberately — `Bearer  gmak_…` presents a leading space. This row
    // exists so nobody "fixes" that by widening the format.
  ...
  test("should refuse every well-formed write key", () => {
    ...
    for (const kind of kinds) {
      ...
      // Non-vacuity: this really is a well-formed key of its own family, for
      // this kind — so the refusal below is about the family, not junk input.
...
describe("hashApiKeyMaterial", () => {
  test("should hash identically to the write-key hash, so the duplication is not a divergence", () => {
    ...
    for (const material of materials) {
      ...
      // The two families sit at different trust levels and own separate
      // functions (D-2) — but there is no behaviour to diverge today, only
      // intent. A future salt/pepper/KDF on either side must be a deliberate,
      // visible change that fails THIS row.
      expect(digest).toBe(hashWriteKeyMaterial(material));
      expect(digest).toMatch(/^[0-9a-f]{64}$/);
      expect(digest).toBe(digest.toLowerCase());
    }

    // A golden vector, computed independently with node:crypto — plain
    // sha256 hex of the material, no salt anywhere in it.
...
describe("API_KEY_DISPLAY_PREFIX_LENGTH", () => {
  test("should define the display prefix as the scheme plus exactly six characters", () => {
    // 11, not `write_keys`' 12: this material is a secret, so half the
    // exposure in scrollback — and the stored value still carries the scheme,
    // so `gmak_x7Kq2p` is self-describing (D-2).
...
describe("apiKeyMetadataSchema", () => {
  test("should describe metadata with no secret field", () => {
    ...
    // Stated separately from the exact list because it is the invariant that
    // must survive any future column: the DTO carries no digest and no
    // material. There is deliberately no `projectId` either (D-1).
