// ADD §9 items 24–25 — session grouping (O-003 FR-26 / F-12 / D12 churn).
//
// F-12's fail direction: toward ONE SESSION PER EVENT for unattributed events.
...
//
// The 30-minute bucket is FIXED, not a sliding inactivity gap: a sliding
// window shifts as late events arrive, forking the key of a session already on
// record. Addendum A ROW 4 pinned that late and backdated events are ordinary
// here, so arrival order must not be able to change a key.
...
describe("deriveSessionKey", () => {
  ...
  test("an empty $session_id or identity key is treated as absent, not as a key", () => {
    // An empty string is a shape PostHog can emit (SEC-C: $session_id is
    // SDK-set). Keying on it would merge every such event into `ph:`.
  ...
  test("the key for the same identity and instant is byte-identical across two derivations and across arrival orders", () => {
    ...
    // A different event id must NOT change the key once an identity exists —
    // otherwise every event would be its own session.
    expect(deriveSessionKey(input({ identityKey: IDENTITY, sourceEventId: "s0-event-2" }))).toBe(
      first,
    );

    // Arrival order is not an input. Addendum A ROW 4 pinned that a backdated
    // event lands behind the newest-first walk, so the same three events read
    // in either direction must produce the same three keys.
    ...
    // And a re-derivation from an equal-but-not-identical Date object is the
    // same string — the key must depend on the instant, not on object identity.
    expect(
      deriveSessionKey(
  ...
  test("SESSION_BUCKET_MS is 30 minutes and the rules are versioned", () => {
    // Persisted per session, so a stamp's provenance is readable from the row
