// ADD §9 items 13, 14, 21, 22, 23 — the exclusion classifier's internal-domain
// predicate (F-3) and its purity/versioning properties (FR-14).
//
// F-3's fail direction: toward INCLUDING as real. Over-exclusion is invisible,
// erases the evidence a finding rests on, and reads to the customer as "nobody
// uses my product". Under-exclusion is visible and cheaply re-marked by the
// later backfill. Asymmetric ⇒ fail open.
...
describe("classifyExclusion — internal domain (F-3)", () => {
  ...
  //
  // Addendum A ROW 6 pinned `person` as null on 165/165 events: email is not
  // on the event at all, so most sessions carry no email and never will. This
  // is what the common request looks like, and it must be KEPT.
  ...
  test('returns "none" when the project has no internal domain to match against', () => {
    // F-2 inferred nothing (free-mail or absent creator email). A session with
    // a real email must not be excluded against a domain we never established.
    expect(
      classifyExclusion(
  ...
  test("matches exactly — acme.com.co and sub.acme.com do NOT match acme.com", () => {
    // A subdomain rule fires on `acme.com.attacker.net`; a suffix rule fires
    // on `acme.com.co`. Both are the D10 superset failure this sprint exists
    // to prevent, so matching is whole-domain equality and nothing else.
    ...
    // Control: the exact domain still excludes, so the test above is not
    // passing merely because the predicate never fires.
    expect(
      classifyExclusion(
        sessionFacts({ identityEmailDomain: "acme.com", internalDomain: "acme.com" }),
        CURRENT_EXCLUSION_RULE_SET,
  ...
  test("an unresolved identity is kept — never laundered into an exclusion", () => {
    // F-8: "we could not check" is not "we checked and it is our own team".
    // The counter reports it separately; the classifier keeps it.
    expect(
      classifyExclusion(
        sessionFacts({
          identityEmailDomain: null,
...
describe("classifyExclusion — reproducibility (FR-14)", () => {
  // Item 21
  ...
  test("rule set version 1 remains resolvable and reproduces a v1 stamp after CURRENT advances", () => {
    ...
    // Every version ever shipped stays in the map. This assertion is what
    // fails when a future contributor edits v1 in place instead of adding v2.
    ...
    // Classified against the explicitly-fetched v1 rules — this expectation
    // stays true when EXCLUSION_RULE_SET_VERSION becomes 2.
    expect(
      classifyExclusion(
        sessionFacts({ identityEmailDomain: "acme.com", internalDomain: "acme.com" }),
        v1,
