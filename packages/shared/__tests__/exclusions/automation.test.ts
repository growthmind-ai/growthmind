// items 15–20, the automation predicates and their near-miss fixtures (F-4 / F-5 / F-6
// / F-7 / F-9, edge taxonomy).
//
...
//
// Every near-miss below is a regression guard against the substring rule a future
// contributor will be tempted to write. `Abbott` / `robotics` / `Botim` are the
// canonical conflation neighbours of a bare `bot` token.
...
/**
 * Classifies on the user agent alone: no email domain and no internal domain, so the
 * only predicate that can fire is an automation one. Keeps every assertion below about
 * the thing it names.
 */
...
describe("automation classification", () => {
  ...
  test("headless and E2E markers are a confident exclude; an ordinary headed Chrome UA is not", () => {
    ...
    // ...and a UA that merely mentions an automation-adjacent word inside a longer
    // token. Word-boundary matching, not substring.
    expect(
      classifyUa(
  ...
  test("Abbott, robotics, and Botim do NOT fire any automation predicate", () => {
    // These exist to fail the day a contributor adds a bare `bot` token. They are
    // ordinary products with ordinary users, and setting their sessions aside would
    // erase real evidence invisibly.
    for (const userAgent of [
    ...
    // The token lists must never contain a bare word that ordinary UAs carry. This is
    // the guard the fixtures above exist to trip.
    for (const list of [HEADLESS_TOKENS, KNOWN_AGENT_TOKENS, CODING_AGENT_TOKENS]) {
      ...
      // Matching lowercases the UA first, so an uppercase token would be a silent
      // never-fires. A stringly-typed no-op, not an error.
  ...
  test("Chrome-Lighthouse and Googlebot fire; an ordinary Chrome UA does not", () => {
    ...
    // Near-misses: the same UA without the auditor suffix, and a token that is only a
    // prefix of an ordinary word.
    expect(classifyUa(HEADED_CHROME_UA)).toBe("none");
  ...
  test('a UA merely containing the word "agent" does not fire the coding-agent predicate', () => {
    ...
    // Control: a real coding agent is still named, and named as its own class so the
    // counter's breakdown can say so in the customer's terms.
  ...
  test('an absent or empty user agent classifies as "none", never as automation', () => {
    // Sec-a: PostHog does not derive a user agent server-side, so a server-side or
    // minimal SDK integration sends none at all. Classifying that as a bot would
    // silently drop real sessions. The exact conflation this sprint exists to prevent.
...
describe("no host-based or domain-pattern predicate exists in src/exclusions", () => {
  ...
  test("no exported symbol names a host, staging, or preview rule", () => {
    // A real early-stage product's production host genuinely IS `something.vercel.app`.
    // Such a predicate fires on a superset of its target by construction, so it is not
    // built, and that is asserted here rather than left as a comment nobody re-reads.
  ...
  test("the exclusion reason union has no host, staging, or preview member", () => {
    // The stamp vocabulary is the contract onward reads. If no reason can express a
    // host exclusion, no writer can smuggle one in.
