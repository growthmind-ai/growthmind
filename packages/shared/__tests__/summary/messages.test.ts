// ADD §9 "Degradation states — enum totality" + the D-10 plain-English audit
// over the one home every customer-facing string this lane produces lives in
// (`packages/shared/src/summary/messages.ts`), modelled on the hostile audit
// at `packages/shared/__tests__/session-source/messages.test.ts`.
//
// This module keeps its OWN `ALL_CUSTOMER_FACING_MESSAGES` rather than
// merging into `session-source/messages.ts`'s export of the same name — T1.1
// is scoped to files under `packages/shared/src/summary/**` and
// `packages/shared/src/env.ts` only, and `session-source/messages.ts` is
// outside that scope. The audit below is therefore total over THIS module's
// exports, the same way O-004's gate messages were total over theirs before
// being folded into the session-source list.
...
/** Every string this lane can put in front of a customer. No templated
 * builders in this module (unlike session-source's), so this is just the
 * fixed table. */
...
describe("the plain-English audit", () => {
  ...
  // CR-10's lesson (O-003): derive the expected set from the module's ACTUAL
  // exports rather than a second hand-maintained list, so a new fixed string
  // constant is picked up automatically the moment it is exported.
...
describe("degradation states — enum totality", () => {
  ...
  // FR-18d — "we have not looked yet" vs "we looked and your product was
  // quiet" (`packages/db/src/services/connection-state.ts:36-50`).
  ...
  // SAC-6, the O-004 regression: read `packages/shared/src/gate/messages.ts:58-85`
  // before touching this test. A degradation sentence keyed by `summary_source`
  // alone is emitted for every cause that member covers, so it may only assert
  // that a written explanation is absent — never a positive claim about what
  // was observed in the customer's product, which the sentence cannot know.
