// ADD §9 item 27 — the counter's freshness statement (O-003 FR-15 / D-6f).
//
// No overlap window can make a poll on client-declared event time complete
// (Addendum A ROW 4: there is no ingestion-time field on the events endpoint by
// any route), so the product states a MEASUREMENT and never claims freshness
// it cannot deliver. The word "live" appears in no string this sprint produces.
...
describe("describeExpectedLag", () => {
  ...
  test("states measurement and never uses the word 'live'", () => {
    ...
    // One home for copy (D-13): the statement is the shared builder's output,
    // not a second string that could drift out of the plain-English audit.
    ...
    // Also true for the accelerated onboarding cadence, so the assertion is
    // about the function rather than one memorised sentence.
  ...
  test("derives both figures from the poll interval plus the measured retrieval lag", () => {
    ...
    // A shorter interval genuinely moves the number — the figures are computed
    // from the connection's own cadence, not a hardcoded pair.
  ...
  test("the worst case is never presented as better than the typical case", () => {
    for (const pollIntervalSeconds of [15, 60, 300]) {
      ...
      // Parses as the shape the counter DTO embeds, so O-008 cannot receive a
      // half-built object.
