// ADD §9 items 43–45 — the impure fetch wrapper.
//
// Addendum A ROW 5's load-bearing fact: the throttle bucket is PER-ENDPOINT.
// While session recordings were throttled, the events list still returned 200.
// So attempt counters live per endpoint for the lifetime of one poll run, and
// a 429 from persons exhausts the identity budget WITHOUT pausing the events
// walk.
//
...
describe("createPostHogClient", () => {
  ...
  test("the 429 loop gives up after MAX_RATE_LIMIT_ATTEMPTS with a recorded reason and never loops unbounded", async () => {
    ...
    // Bounded: the attempt counter stops exactly at the cap, and the request
    ...
    // It backed off between attempts rather than hammering, and every wait
    // honoured the server's stated 59 seconds.
  ...
  // Item 44 — FR-10. A grep guard, so the bound above cannot be reintroduced
  // as an unbounded loop somewhere else in the package.
  ...
  // Item 45 — ROW 5's per-endpoint bucket; mitigates the ASSUMED §7 row for
  // the persons throttle profile, which was never load-tested.
  ...
  // Item 45, the other direction: two healthy endpoints keep independent
  // counters, so the split above is a real separation and not an artefact of
  // one endpoint never being touched.
