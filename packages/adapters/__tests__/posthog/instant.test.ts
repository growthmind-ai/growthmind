...
describe("formatPostHogInstant", () => {
  ...
  test("emits an explicit +00:00 offset and never a naive string", () => {
    ...
    // A `Z` suffix is not what the API echoes; a naive string is worse still. It is
    // parsed as UTC and truncated to whole seconds, which measurably changed a result
    // set (8 rows instead of 7).
    expect(formatted.endsWith("Z")).toBe(false);
    expect(formatted).toContain("+00:00");
  });

  // Item 32, F-10: the output is gated by its own pattern before it can reach a
  // request, so nothing unvalidated ever lands on `after` or `before`.
...
describe("parsePostHogInstant", () => {
  ...
  test("parses the API's microsecond +00:00 form and does not string-compare against toISOString", () => {
    ...
    // The proof that nothing in this adapter may string-compare a timestamp: the
    // round-trip is not byte-identical to the wire form.
    expect(parsed?.toISOString()).toBe("2026-07-30T17:57:49.891Z");
    expect(parsed?.toISOString()).not.toBe(wire);

    // An unparseable value is a value the caller must handle, never a NaN Date that
    // propagates silently into a request parameter.
