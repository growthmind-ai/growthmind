// ADD §9 items 39–42 — pure backoff. No clock, no I/O, no randomness of its
// own: `random` is injected, so both jitter branches are asserted EXACTLY and
// nothing in this file waits.
//
// Addendum A ROW 5: `Retry-After` is present on a PostHog 429 and its value is
// BARE DELTA-SECONDS ("59"), never an HTTP-date. It is also the only
// rate-limit header, so headroom is invisible until the limit trips.
...
describe("parseRetryAfterSeconds", () => {
  ...
  test("parses a bare integer and rejects an HTTP-date and garbage", () => {
    ...
    // The HTTP-date form the RFC also allows was NEVER observed here. Reading
    // it as a number would yield NaN or a nonsense delay, so it takes the
    // independent exponential fallback instead.
...
describe("computeBackoffDelayMs", () => {
  ...
  test("honours Retry-After up to the cap and never below the server's instruction", () => {
    // Jitter on this branch is ADDITIVE-UPWARD ONLY: never earlier than the
    // server instructed.
  ...
  // Item 41 — FR-10. The fallback is retained deliberately: only ONE
  ...
  test("falls back to capped exponential with full jitter when Retry-After is absent", () => {
    // BASE_DELAY_MS * 2^(attempt-1), then full jitter `delay * (0.5 + 0.5r)`.
  ...
  // Item 42 — FR-10. The asymmetry between the two branches IS the point:
  // full jitter may retry early, which is fine against a limit we inferred and
  // not fine against one the server stated.
  test("jitter bounds are exact for random=0 and random=1 on both branches", () => {
    ...
    // Retry-After branch, 10 s ⇒ never earlier than 10 s, never later than
    // 10 s + one jitter spread.
