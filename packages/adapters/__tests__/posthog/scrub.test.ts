// ADD §9 item 49 — FR-7's named strengthening.
//
// The spike's `scrubKeys` redacts only EXACT whole-string occurrences of the
// values the process holds, so a key echoed back URL-encoded, JSON-escaped, or
// truncated survives it untouched. The production scrubber runs both passes:
// the exact values (and their encoded variants) first, then a pattern pass
// over PostHog's own `ph?_…` key shapes.
...
describe("scrubSecrets", () => {
  ...
  test("redacts a URL-encoded key and a truncated key that exact-value scrubbing would miss", () => {
    // (a) URL-ENCODED. `AD_FAKE_ENCODABLE_KEY` is deliberately chosen so its
    // pattern-matchable run is shorter than POSTHOG_KEY_PATTERN's 16-char
    // minimum — only the exact-value pass and its encoded variants can catch
    // it, which is precisely the case exact whole-string scrubbing misses.
    ...
    // (b) TRUNCATED / UNKNOWN. This value was never handed to the scrubber as
    // a secret, so only the pattern pass can catch it.
    ...
    // (c) The exact-value pass still works, and ordinary prose is untouched —
    // over-redaction costs nothing, but a scrubber that eats every message is
...
describe("truncateForReason", () => {
  // Supports item 49: a scrubbed reason still has to fit the stored column,
  // and truncation must never re-expose a value by cutting mid-escape.
