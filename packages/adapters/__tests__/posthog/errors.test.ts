...
// header is invalid." is exactly the jargon the plain-English bar forbids.
...
describe("mapFailure", () => {
  ...
  test("a 401 maps to invalid_credentials by envelope code, and no key material appears in the message", () => {
    ...
    // A missing Authorization header is the same 401 with a different `code`. Branching
    // on the status alone could not tell these apart.
    expect(mapFailure(401, AD_NO_HEADER_BODY).code).toBe("invalid_credentials");

    // Nothing that could carry the customer's key ever reaches a customer-facing or
    // stored string.
  ...
  // Item 47 —: three different things to fix, told apart.
  test("an authenticated read against a wrong project maps to project_not_found, distinctly from invalid_credentials and unreachable", () => {
    ...
    // Pairwise distinct in both the code and the message. A screen must never present
    // two different fixes the same way.
  ...
  // The wiring itself. Nothing in `SOURCE_FAILURE_MESSAGES` is expected to ever contain
  // a secret (the messages are a fixed, hand-written set), so this proves `mapFailure`
  // actually calls `scrubSecrets` on the message it builds, rather than merely
  // accepting an unused `secrets` parameter. A substring lifted straight out of the
  // `unreachable` sentence stands in for "a value the caller told `mapFailure` to treat
  // as a secret".
  test("mapFailure scrubs every secret it is handed out of the returned message", () => {
    ...
    // (`config.personalApiKey`) is scrubbed the same way, even though it structurally
    // never appears in a message today.
