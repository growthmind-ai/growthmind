// ADD §9 items 61 and 63 — the port itself (FR-2, D-11).
//
// Two methods, not three. Health is persisted state on the connection row,
// derived by the service from the last validate/pull result — a `health()`
// method would create a second source of truth for something the database
// owns and would reintroduce the D4 failure of gating on a transient signal.
...
describe("the SessionSource port", () => {
  // Item 61 — FR-2 / D-11. The vendor name must not leak past the composition
  // root, and a second adapter must be a COMPILE error (an unhandled branch of
  // the one-member union), never a silent fallthrough in a lookup table.
  test("exactly one SessionSource implementation is referenced, by name — no registry, factory map, or dynamic lookup", () => {
    ...
    // (a) Exactly one function in the package returns a `SessionSource`.
    ...
    // (b) The barrel names it, so the composition root imports a symbol rather
    // than resolving a string.
    const barrel = files.find((file) => file.path === "index.ts");
    expect(barrel).toBeDefined();
    expect(barrel?.code).toContain("createPostHogSessionSource");

    // (c) No registry, factory table, or dynamic lookup anywhere.
  ...
  // Item 63 — FR-2. The port must be drivable with no network at all: by a
  // hand-written fake, AND by the real implementation through injected deps.
  test("the port is exercisable against a fake with no network", async () => {
    // (a) A fake with no network satisfies the port and returns port-valid
    // shapes. This is the substitutability guarantee `packages/db`'s connection
    // service depends on — it injects a source factory precisely so it can be
    // tested against this.
    ...
    // (b) The REAL implementation is constructed from injected effects only —
    // a fetch that would fail loudly if the port needed a live connection just
    // to be built, and a `sleep` that never waits.
