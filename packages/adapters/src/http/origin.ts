...
// Null for any address a credential must never be sent to. `posthog/host-guard.ts`
// resolves the same policy into the reason its customer is shown.
...
export function isSameOriginAsHost(candidate: string, host: string): boolean {
  const configured = safeOrigin(host);
  if (configured === null) return false;

  return safeOrigin(candidate) === configured;
}
