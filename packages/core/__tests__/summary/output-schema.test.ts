// The PARITY suite for the SAC runtime guard (O-011 AD-6/AD-7/AD-8, FR-M3/FR-M4).
//
// WHY THIS FILE EXISTS. `guards.test.ts` pins four mechanical scanners, each
// proven non-vacuous by a planted offender — but those scanners live in a TEST
// file, and a test file is not an import surface. AD-6 therefore RE-EXPRESSES
// them in production inside `src/summary/output-schema.ts`, and accepts the
// resulting duplication on one condition: that a scanner which stops biting
// fails a NAMED test here (ADD risk R-4). So the planted offenders below are
// taken VERBATIM from `guards.test.ts` — not paraphrased — and replayed against
// the production `guardModelText`. If someone edits a scanner in one home and
// not the other, this suite goes red rather than the model lane going quiet.
//
...
/** The surface the planted offenders name. `guards.test.ts:459` plants
 * "…reached /pricing…", so the candidate must legitimately be ABOUT /pricing —
 * otherwise a replay would be rejected by SAC-4 (a foreign path) and the
 * scanner under test would never be reached. */
const FIXTURE_SURFACE = "/pricing";

/** A headline that is clean under every SAC row, so a rejection below is
 * attributable to the planted context and to nothing else. */
 ...
 *
 * `source` names the line the string was taken from, so a drift review can
 * diff the two files by eye. `counts` is the candidate context each offender
 * needs to be an offence at all — the denominatorless plant is only an offence
 * relative to a 20-of-30 count, exactly as `guards.test.ts:465-472` states it.
 */
 ...
 *
 * Not every member need be unsegmentable — production owns that rule — but the
 * suite asserts at least one IS, so "returns null" can never quietly become
 * "never returns null" and take FR-M4's whole fail direction with it.
 */
const UNSEGMENTABLE_CANDIDATES: readonly string[] = [
  // No terminator anywhere: one unbounded run-on, so no element boundary exists
  // to judge SAC-3 and SAC-11 per sentence.
  "people kept coming back to /pricing and then the save did not work and nothing else happened",
  // A terminator that does not end a sentence — segmenting on "." here bleeds a
  // clause across two elements, which is precisely what AD-6 forbids.
 ...
 * `shared/src/summary/types.ts:188-191` says FR-8 "rests on this comment
 * alone"; `.strict()` is what turns that comment into structure. */
...
describe("model summary output schema and SAC guard", () => {
  test("the SAC guard rejects every planted offender the shipped scanner suite reports", () => {
    ...
    // The CONTROL: clean text on the same candidate passes. Without it, a guard
    // that rejected everything would score full marks below.
    ...
    for (const planted of PLANTED_OFFENDERS) {
      ...
      // Reported as {source, ok} rather than a bare boolean: a parity failure
      // must name WHICH shipped plant stopped biting.
      ...
      // An offence names the rule and the element index — NEVER the offending
      // text, which carries a customer's page path and counts (AD-7).
  ...
  test("prose that cannot be segmented into single sentences is withheld rather than judged", () => {
    ...
    for (const text of refused) {
      ...
      // WITHHOLD, never publish-unchecked (D10, FR-M4): unsegmentable prose is
      // itself a rejection, so it falls to the floor as
      // `floor_model_text_rejected` rather than reaching a customer unjudged.
      expect({ text, ok: verdict.ok }).toEqual({ text, ok: false });
    }

    // The segmentation that DOES succeed is single-sentence per element and
    // round-trips through the join — otherwise per-sentence rules (SAC-3,
    // SAC-11) would be judged over clauses that bled across elements.
  ...
  test("the model output schema refuses a field the contract never declared", () => {
    ...
    for (const key of UNDECLARED_KEYS) {
      ...
      // `.strict()` is load-bearing: an extra key is a REFUSAL, not a silent
      // drop. A silent drop would let a model emit a number or a class name and
      // leave no trace that it tried.
    ...
    // The declared fields are both required and non-empty: an empty headline is
    // a shape failure (`floor_model_output_invalid`), not text for the guard.
