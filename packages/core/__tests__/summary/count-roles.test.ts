// The five named tests for the count-role resolver (FR-F4, O-005 D-5).
//
// WHAT THIS FILE IS FOR, in one sentence: `CandidateFinding.counts` is a
// positional array whose roles live only in prose at two emission sites, and
// this suite is what stops the table that describes those sites from drifting
// away from them.
//
// THE LOAD-BEARING TEST IS THE FIRST ONE. It runs the REAL detectors over
// firing corpora and compares each produced candidate's arity against the
// declared arity. A table checked only against itself would agree with itself
// forever; this one disagrees with the code the moment either detector's
// emission changes, without anybody having to remember to edit it.
//
...
// ---------------------------------------------------------------------------
// Frozen fixture time and vocabulary — all `t1cr`-prefixed, colliding with no
// other suite
// ---------------------------------------------------------------------------
...
/** Sessions above the origin floor, so the fixture is not sitting on a
 * boundary this suite is not about. */
const FUNNEL_HEADROOM_SESSIONS = 10;
/** One session past the inclusive rate gate, for the same reason: the
 * boundary's inclusivity is `funnel-dropoff.test.ts`'s subject, not this
 * file's, and a fixture sitting exactly on it would make these arity claims
 * hostage to a decision they do not test. */
...
// corpus and nothing in this file turns on the set-aside rule.
// ---------------------------------------------------------------------------
 ...
 * hand-tuned: enough sessions at the origin to clear `funnelMinSessionsAtOrigin`
 * with headroom, and enough of them ending there to clear both
 * `funnelMinDropoffSessions` and the inclusive rate gate.
 *
 * Deliberately asymmetric — strictly more sessions reach the origin than leave
 * it without continuing — so a resolver that swapped the two roles would be
 * caught by an assertion rather than hidden by equal numbers.
 */
...
function errorSession(index: number, ruleSet: ThresholdRuleSet): SessionTimeline {
  ...
  const startedAt = sessionStartedAt(index);
  // Well inside `errorCorrelationWindowMs`, so the exception correlates to the
  // action that preceded it and the fixture is the ordinary firing case.
 ...
 * `candidateFindingSchema`, so the fixture cannot drift from the contract and
 * its counts are genuine branded `MeasuredCount`s rather than hand-built shapes
 * the resolver would never see in life.
 *
 * `counts` may be overridden — that is the only way to express the arity
 * disagreement the refusal test needs, since neither detector can produce one.
 *
 * The trace, the evidence shape and the ranking are here only because the
 * contract requires them. `resolveCounts` reads NONE of them; it reads
 * `detector` and `counts` and nothing else.
 */
...
/** Every digit run in a string, so an error message can be audited for values
 * it must not carry rather than eyeballed. */
...
describe("count roles", () => {
  // FR-F4c. THE SPRINT'S ONE REAL WIRE TO THE DETECTORS, and the only test here
  // that is behavioural rather than a statement about the table. Everything
  // else in this file could pass against a table that describes detectors that
  // do not exist; this one cannot.
  test("every detector's declared count roles match the arity its detector actually produces", () => {
    ...
    // --- NON-VACUITY, BEFORE ANY ARITY CLAIM -------------------------------
    // An empty corpus makes "every produced candidate has the declared arity"
    // trivially true, which is the exact way this test would rot into theatre.
    for (const result of results) {
      expect(result.candidates.length).toBeGreaterThan(0);
    }

    // ...and BOTH detectors are exercised, not one of them twice. Derived from
    // the schema, so a third detector added without a corpus here fails rather
    // than escaping the check.
    ...
    // The two declared arities DIFFER, so the check above cannot be satisfied
    // by a table that happens to state one number everywhere.
    expect(COUNT_ROLES.funnel_dropoff.length).not.toBe(COUNT_ROLES.error_event.length);
  });

  // FR-F4d. Fail direction: refuse.
  test("a candidate whose counts arity disagrees with its detector's declared roles is refused", () => {
    ...
    // TOO MANY: an error candidate carrying a second count it has no role for.
    // Neither detector can produce this, which is why the fixture builds it.
    ...
    // It refuses rather than truncating or padding: no partial map is returned
    // on either side of the declared arity.
    ...
    // ...AND NO COUNT VALUE. Nothing about somebody's product reaches a log
    // line. The precondition is asserted first, so this is not satisfied by a
    // fixture whose numbers happen to be the arities.
    const values = [
  ...
  // FR-F4b, funnel arm. THE HAZARD, NAMED: `counts[0]` is the arrival count and
  // `counts[1]` is the departure count, and only the emission site said so.
  test("resolveCounts returns each count under its declared role for a funnel candidate", () => {
    ...
    // `toBe`, not `toEqual`: the resolved role must hold the very count object
    // at that position, so a resolver that rebuilt a lookalike would fail.
    ...
    // ...and the two roles are not interchangeable. The fixture is built so
    // strictly more sessions reached the origin than left it without
    // continuing, so a resolver that swapped the roles fails here rather than
  ...
  // FR-F4b, error arm. The arity the positional read gets wrong in the other
  // direction: `counts[1]` is `undefined` for every candidate this detector
  // produces, so there is no second role to reach for.
  ...
  // FR-F4a. The compile pin is `satisfies Record<DetectorName, ...>`; this is
  // its runtime observation, in BOTH directions — a detector with no row, and a
  // row naming no detector.
  test("COUNT_ROLES has an entry for every detector name", () => {
    ...
    // Stated in both directions explicitly, so a reader does not have to derive
    // that a sorted equality is the conjunction of the two containments.
    for (const name of known) expect(declared).toContain(name);
    for (const name of declared) expect(known).toContain(name);

    // Every row declares at least one role, and no row names a role twice — a
    // duplicate would pass the resolver's arity check and then silently drop a
    // magnitude the resolved type promises.
