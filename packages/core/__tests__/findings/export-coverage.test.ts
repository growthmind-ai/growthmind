// FR-J (P1) — the self-enumerating export-coverage guard for O-006's five
// modules (ADD tasks/signature-ledger/add.md §7 "Self-enumerating guard").
//
// WHY THIS EXISTS. A previous sprint's version of this guard named two
// missing test titles verbatim on resume from a dead, context-exhausted
// agent, which turned a lost session into a two-line brief instead of a
// re-audit of the whole diff. The value of this file is ENTIRELY in the
// quality of its failure message — a bare "3 exports lack tests" would have
// been useless for that resume; "Missing test for `resolveSignature` —
// expected a test titled `should resolve …`" is what makes the guard worth
// having. Every failure path below names the missing test title VERBATIM.
//
// WHY THIS FILE LIVES IN `packages/core/__tests__`, NOT `packages/db`.
// O-006's five modules span two packages, and `packages/core -> packages/db`
// is a forbidden import direction (ADD C-c) — a test file is bound by the
// same rule if it imports the OTHER package's runtime code. But this file
// never imports `@growthmind/db`; it reads db's source and test files off
// disk with `node:fs`, exactly the way `packages/core/__tests__/detect/
// purity.test.ts` reads its OWN package's files with `Bun.file` to scan them
// without becoming an import. A disk read is not a module import — nothing
// resolves through Node's module graph, nothing crosses the dependency arrow
// this ADD polices. `packages/core` is the chosen home (rather than
// `packages/db`) because the ADD's §7 checklist already pins this exact path
// (`packages/core/__tests__/findings/export-coverage.test.ts`) and because
// `packages/core/src/findings` is where the identity half of the ledger's
// contract is defined; the guard reading outward from there to verify the
// persistence half (`packages/db`) is no different in kind from
// `no-org-param.test.ts` reading two directories (`repositories/`,
// `services/`) from one test file.
//
// WHAT COUNTS AS "AN EXPORTED FUNCTION" HERE. Only top-level
// `export function` / `export async function` declarations — never a type,
// an exported `const`, or a method signature declared inside an exported
// `interface`. The five service entry points (`recordSignature`,
// `consultSignature`, `markSignatureDelivered`, `recordDismissal`,
// `recordAncestry`) and the four repository methods on each of the three
// repos are `SignatureLedgerService`/`*Repo` INTERFACE methods, implemented
// as object-literal properties inside their factory's returned object — they
// are not standalone exported functions, so this guard does not enumerate
// them individually. Every one of them is still covered: T-E2E-1 and the
// per-repository integration suites exercise all nine through the one
// function this guard DOES enumerate for each file — the factory
// (`createFindingSignaturesRepo`, `createDismissalsRepo`,
// `createSignatureAncestryRepo`, `createSignatureLedgerService`).
//
// HOW A MISS IS DETECTED, NOT JUST A COUNT. `EXPECTED_TEST_TITLES` is a
// manifest — export name -> the literal, currently-passing test title that
// proves that export is tested (titles copied verbatim from the real test
// files, verified present before this guard shipped). The guard:
...
/** `packages/db` — a sibling package root, read from disk only, never
 * imported (see header). */
...
/**
 * Blanks `//` and `/* *‍/` comments to spaces (preserving length and
 * newlines, so line-anchored regexes still line up), while leaving string
 * literals untouched — a title inside a string must survive; prose inside a
 * comment claiming to declare an export or a test must not be mistaken for
 * one. Ported from the `no-org-param.test.ts` / `purity.test.ts` precedent's
 * `stripSource`, narrowed to the one view (`withoutComments`) this file
 * needs for both of its scans.
 */
function stripComments(source: string): string {
  ...
  while (cursor < source.length) {
    ...
    if (quote === '"' || quote === "'" || quote === "`") {
      // Skip over the string's content untouched — titles and specifiers
      // live inside these and must survive for the caller to read.
      let scan = cursor + 1;
...
/** A top-level `export function` / `export async function` declaration's
 * name, anchored at the start of a line (with only leading whitespace) so an
 * indented object-literal method (e.g. a repo factory's returned
 * `async upsertSeen(...) {`) can never match — those are interface
 * implementations, not exported function declarations (see header). */
...
/** Every literal string title passed as the first argument to `test(...)` or
 * `it(...)` in a test file — quote-agnostic (`"`, `'`, or `` ` ``). */
...
/** Must be found: a real top-level exported function. Must NOT be found: a
 * non-exported helper, an interface method, and a commented-out export that
 * only LOOKS like a declaration once comments are stripped incorrectly. */
...
// ===========================================================================
// PART 2 — the six O-006 modules, and the manifest of what proves them tested
// ===========================================================================
...
/**
 * export name -> the literal test title that proves it is tested, copied
 * VERBATIM from the real, currently-green test files (confirmed present
 * before this guard shipped). Every title here is a `test(...)`/`it(...)`
 * call's first argument, character-for-character.
 */
...
describe("export coverage guard — every O-006 exported function has a named test (FR-J)", () => {
  it("enumerates more than zero exports across the O-006 modules and finds signatureTuple and suppressionDecision by name", () => {
    const allExports = MODULES.flatMap((mod) =>
      collectExportedFunctionNames(readFileSync(mod.sourcePath, "utf8")),
    );

    // Non-vacuity (FR-J c): if the scan silently matched nothing — a wrong
    // path, a regex that stopped matching this codebase's style — every
    // assertion below would pass on an empty set and mean nothing.
  ...
  it("declares no exported O-006 function the manifest and its test file cannot account for", () => {
    ...
    for (const mod of MODULES) {
      ...
      // Anti-vacuity per module: a wrong `sourcePath` reads real bytes but
      // finds zero exports, which would silently drop that file out of the
      // guard's coverage with no assertion noticing.
      ...
      for (const exportName of exportedNames) {
        ...
        if (expectedTitle === undefined) {
          ...
          // "self-enumerating" half of the guard: it does not require
          // someone to remember to update a checklist when a seventh
          // function is added to one of these six files — the scan finds
          // it, and its total absence from the manifest IS the failure.
          offenders.push(
        ...
        if (!actualTitles.has(expectedTitle)) {
          // THE requirement this guard exists for: name the missing test
          // title VERBATIM, not a count.
          offenders.push(
  ...
  it("carries no manifest entry for a function no O-006 module exports anymore", () => {
    // The other direction of manifest rot: an export renamed or removed
    // leaves a permanently-vacuous row that can never fail, silently
    // shrinking the guard's real coverage while its test count looks
    // unchanged.
