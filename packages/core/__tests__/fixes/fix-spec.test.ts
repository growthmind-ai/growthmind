// The named tests for the minimal fix spec (O-009).
//
// WHAT THIS FILE IS FOR, in one sentence: `renderFixSpec` is the first code in
// this repository that puts words in front of a CODING AGENT — something with
// write access to somebody's repository — and this suite is what holds it to
// describing what is wrong rather than prescribing an edit.
//
// THE ASSERTIONS ARE WRITTEN INDEPENDENTLY OF THE MODULE'S OWN GUARD. The
// no-code test below does NOT call `isCodeShaped` to prove the output is
// clean — a guard checked against itself is a mirror, not a gate. It carries
// its own literal markers (a fence, `@@`, `+++`, an edit instruction, a
// `file.ts:42`), so a marker deleted from `CODE_SHAPED_MARKERS` to make a
// template pass still fails here. `isCodeShaped` is separately proved to
// FIRE on real code, so it is not merely blind.
//
 ...
 * `../../src/delivery/slack-message.ts` asserts `describesPeople` returns
 * `true` for, as INTENDED behaviour. It is a customer's own URL, and the whole
 * point of the people guard scanning templates rather than rendered sentences
 * is that this renders verbatim.
 */
...
/**
 * The ES-7 basis: every session in the window was SET ASIDE, so `kept` — and
 * therefore the denominator — is zero. Distinguishable by construction from
 * "nothing happened in the window", which would have `totalInWindow` zero too.
 */
...
/**
 * A `CandidateFinding` PARSED through `candidateFindingSchema`, so no fixture
 * here can drift from the contract the renderer is typed against. Every field
 * a test varies is a named parameter; nothing is silently defaulted into a
 * shape the renderer would never meet.
 */
...
/**
 * The workhorse fixture: a funnel finding on a surface, with two counts in
 * declared role order and deliberately DIFFERENT numerators, so a renderer that
 * swapped the two magnitude sentences is caught by an assertion rather than
 * hidden by equal numbers.
 */
...
/** One signal of every kind in the union, so a completeness claim is made over
 * the real enumeration rather than over the two kinds a fixture happened to
 * carry. */
...
/**
 * Code markers written HERE, not imported from the module under test. If a
 * marker were deleted from `CODE_SHAPED_MARKERS` so a template could pass, the
 * output would still fail this list.
 */
 ...
 * deliberately including the shapes a well-meaning later edit would reach for
 * first ("change X to Y", "you should update…").
 *
 * The object may be one to four words, not exactly one: "change the retry limit
 * to 5" is the shape a real instruction takes, and a pattern that only saw
 * "change X to Y" would step straight over it.
 */
...
/** Real code, real diffs, real instructions — the corpus that proves both this
 * suite's scans and the module's own guard are not simply blind. */
...
/** Every fixed string this module authors, plus every sentence a spec renders —
 * the union an audit has to cover for "total" to mean anything. */
function everyRenderedSentence(specs: readonly FixSpec[]): readonly string[] {
  return specs.flatMap((spec) => spec.sentences);
}

/** The specs the broad audits scan: both detectors, with and without signals,
 * on an ordinary surface and on one carrying a cohort noun. */
...
describe("renderFixSpec — structured state as plain sentences (O-009)", () => {
  // --- the shape ----------------------------------------------------------
  ...
  test("should render every section non-empty for a finding with evidence", () => {
    ...
    // `sentences` is DERIVED, so a section cannot be added to the spec and
    // quietly omitted from the flattening an audit and a caller both read.
  ...
  test("should parse its own output through fixSpecSchema", () => {
    ...
    // The hand-written `FixSpec` type and the runtime schema are two statements
    // of one contract. This is what stops them drifting: a field added to the
    // type but not the schema renders unvalidated, and a field the schema
    // requires that nothing produces fails here.
  ...
  test("should carry a denominator in the same sentence as every numerator it renders", () => {
    ...
    // Both magnitudes, in DECLARED ROLE ORDER — the arrival count first, the
    // departure count second. Asserted with different numerators so a swap is
    // visible.
    ...
    // ...and the unit is SESSIONS, never people. `MeasuredCount.unit` is the
    // literal `"sessions"` because identity stitching does not exist in this
    // product, and the rendered sentence must not enlarge the claim.
  ...
  test("should render the explicit no-rate sentence, never a percentage, when the denominator is zero", () => {
    const spec = renderFixSpec({
      candidate: candidateOf({
        ...
        finalClass: "broken",
        surface: ERROR_SURFACE,
        // ES-7: everything in the window was set aside, so `kept` — and the
        // denominator — is zero. A real, reportable state, not an error.
        counts: [countOf(0, allSetAsideBasis(SET_ASIDE))],
        confidenceBasis: "below_threshold",
        coverage: CLEAN_COVERAGE,
      }),
  ...
  test("should render a complete, honest spec when the finding carries no evidence signals", () => {
    ...
    // NOT an empty section and NOT a crash: the honest sentence, then the
    // counts the spec still stands on.
    expect(spec.evidence).toEqual([FIX_SPEC_NO_EVIDENCE_TEMPLATE]);
    expect(spec.symptom.length).toBeGreaterThan(0);
    expect(spec.measurement.length).toBeGreaterThan(1);
    expect(spec.boundary.length).toBeGreaterThan(0);

    // The evidence LIMITATION sentence qualifies evidence lines, so with none
    // rendered it must not appear — it would refer to nothing.
  ...
  test("should emit one sentence per distinct observation when a finding repeats one signal kind", () => {
    ...
    // D3 multiplicity. Three signals, one observation — these sentences are
    // qualitative and carry no per-signal magnitude, so emitting the identical
    // sentence three times would state one observation as though it had been
    // made three separate ways.
  ...
  test("should have an evidence sentence for every member of the evidence signal union", () => {
    ...
    // The table is `Record<EvidenceSignalKind, string>`, so a sixth kind added
    // without its sentence is a compile error. This is the other direction: a
    // key here with no member behind it in the union.
  ...
  test("should render a surface containing a cohort noun verbatim, never rewriting a customer's own page address", () => {
    // Non-vacuity, and the whole reason this test exists: the guard really does
    // fire on this value. `../../src/delivery/slack-message.ts` asserts the same
    // thing as INTENDED behaviour, and a path is not a claim about people.
    ...
    // ...and the spec still renders in full. A guard that fired on the path
    // could only respond by rewriting it, dropping the sentence, or refusing
    // the spec — all three worse than the thing prevented.
  ...
  test("should never pair a count with a cohort noun in any rendered sentence", () => {
    ...
    // A sentence carrying a numerator-and-denominator pair may not describe
    // human beings: identity stitching does not exist in this product, so
    // "12 of 25" is 12 of 25 SESSIONS. The scan excises the surface first —
    // the customer's own URL is not our prose, and is exactly what must NOT be
    // policed (the composed-input rule, D10).
  ...
  test("should contain no code fence, no diff marker and no file-and-line reference in any rendered sentence", () => {
    ...
    // ...and the markers really fire on real code, so a clean report below is
    // a fact about the output rather than about a broken scan.
    ...
    // We dispatch a spec, not a patch. A fix spec carrying a diff is the
    // product doing the one thing it promised not to do, handed to an agent
    // with write access to somebody's repository.
    expect(offenders).toEqual([]);
  });
  ...
  test("should contain no instruction phrased as an edit in any fixed string or rendered sentence", () => {
    ...
    // The spec describes what is wrong and what the evidence is. What to change
    // is the agent's decision, and stating it here would be a claim nothing
    // measured — this module has never read a line of the customer's source.
    expect(offenders).toEqual([]);
  });

  test("should refuse to render a template that reads as code rather than shipping it", () => {
    // The module's own guard, proved to FIRE before it is trusted to report
    // clean. A guard checked only against the strings it passes is a mirror.
    expect(CODE_SHAPED_MARKERS.length).toBeGreaterThan(0);
    for (const sample of REAL_CODE_SAMPLES) {
      expect(isCodeShaped(sample)).toBe(true);
    }

    // ...and it passes every fixed string this module actually ships, so the
    // strictness above is not achieved by refusing everything.
    for (const template of FIX_SPEC_ALL_TEMPLATES) {
      expect(isCodeShaped(template)).toBe(false);
    }

    // No marker carries the `g` flag: a global pattern is stateful across
    // `.test()` calls, so the same template would match on one render and not
    // the next — a determinism bug inside the guard that exists to make the
  ...
  test("should contain no product jargon in any fixed string or rendered sentence", () => {
    ...
    // The banned list is the PRD's, in full — imported rather than restated, so
    // a word added to `FORBIDDEN_PRODUCT_JARGON` tightens this scan for free
    // and a shortened copy cannot make it pass by scanning for less.
    const banned: readonly string[] = FORBIDDEN_PRODUCT_JARGON;
  ...
  test("should render byte-identically for the same input twice and for two structurally identical inputs", () => {
    // (a) THE SAME OBJECT GRAPH, TWICE. Catches a renderer that mutates its
    // input, memoises across calls, or lets an accumulator survive.
    ...
    // (b) TWO DISTINCT BUT STRUCTURALLY IDENTICAL GRAPHS — the half that would
    // catch a clock, a random source, an id counter, or an iteration order
    // derived from object identity. None of those can hide in (a).
    ...
    // The window is rendered from the finding's own timeframe, so no clock is
    // read and these are the ONLY two dates any rendered string may contain.
  ...
  test("should state what the run could not see when the read was truncated", () => {
    ...
    // D-3: a limitation carried onto every finding may never be silently
    // dropped. A spec that hid a truncated read would hand an agent a floor
    // while presenting it as a total.
    ...
    // ...and a clean run says neither, rather than saying "nothing was missed"
    ...
    // The bare `eventsWithoutUrlPath` number never reaches the reader: it is a
    // count with no denominator, and a bare number in front of a reader is the
    // one thing `MeasuredCount` exists to prevent.
  ...
  test("should refuse a surface that is not already in its normalised form, naming no input value", () => {
    ...
    // FAIL DIRECTION: REFUSE. `candidateFindingSchema.surface` accepts any
    // non-empty string, so the contract alone does not prove the redaction ran,
    // and a fix spec is handed to an agent that may log it.
    expect(() => renderFixSpec({ candidate, signals: [] })).toThrow(
      /fix_spec_surface_not_normalised/,
    );

    // THE REFUSAL NAMES ONLY THE NORMALISED FORM. Naming the input would put
    // the very query string this check exists to stop into a log line.
    try {
  ...
  test("should refuse a finding whose counts do not match its detector's declared roles", () => {
    // An error finding declares ONE count; this one carries two. Refusing is
    // the safe direction: truncating drops a magnitude silently, padding
    // invents one, and either puts a number that means something else in front
    // of an agent about to change somebody's product.
    const candidate = {
      ...errorCandidate(),
  ...
  test("should refuse a finding that does not satisfy the input contract, before rendering anything", () => {
    // THE BOUNDARY COMES FIRST, so a caller cannot skip it by passing an
    // already-typed value. `evidenceShapeVersion` is `z.number().int().positive()`
    // on the contract and the type says only `number`, so this is a shape the
    // compiler accepts and the schema must refuse — exactly the gap a parse at
    // the door exists to close.
    const candidate: CandidateFinding = { ...funnelCandidate(), evidenceShapeVersion: 0 };

    expect(() => renderFixSpec({ candidate, signals: [] })).toThrow();

    // ...and nothing was rendered on the way to the refusal: the parse is the
    // first statement in the function, before the surface check, before the
    // count resolution, and before any template is read.
