...
// ADD tasks/session-source-posthog-adapter/add.md §9 items 73–74.
//
// The session upsert is the one place a retried worker task can corrupt data,
// so its conflict resolution is the contract: `started_at` takes the EARLIEST,
// `last_event_at` the LATEST, the email domain is never erased once known, and
// `identity_resolution` upgrades monotonically along
// `unresolved → absent → resolved` (D-5/F-8). "We could not check this time"
// must never overwrite "we checked and found an email last time".
//
// `createSessionsRepo` is a typed-stub throw today, so every test fails on
// "not implemented".
...
/** Seeds an org + project + connection and returns everything a session row
...
describe("sessions repository", () => {
  ...
  it("returns the persisted rows from an upsert so the caller can key events to them", async () => {
    ...
    // Re-applying the same batch must return the SAME row ids, or a retried
    // worker task would orphan the events it keyed to the first run.
  ...
  it("never regresses identity resolution from resolved back to absent or unresolved", async () => {
    ...
    // A later run whose identity budget was exhausted reports "unresolved".
    // That is "we could not check", not "there is nothing" — it must not
    // erase what an earlier run established (F-8).
  ...
  it("keeps a completed absent lookup rather than downgrading it to unresolved", async () => {
    ...
    // `absent` is a FACT (a completed lookup proving no email); `unresolved`
    // is an admission of ignorance. The fact wins.
    expect((await repo.findByKey(projectId, sessionKey))?.identityResolution).toBe("absent");
  });
