...
// ADD tasks/session-source-posthog-adapter/add.md §9 items 64–69.
//
// Every assertion targets the PUBLIC contract of `createProjectConnectionsRepo`
// and `toConnectionSummary` against REAL SQL via PGlite — a fake would prove
// nothing about the partial unique index (FR-8/D6) or the `(org, id)` mutation
// key (FR-19/D7) that this ADD rests on.
//
// The whole repository is a typed-stub throw today, so every test below fails
// on "not implemented". They are written so the stub's generic throw can never
// SATISFY them: each one reads state back through the repository after the
// call, which an unimplemented factory can never reach.
...
/**
 * An obviously-fake, envelope-SHAPED literal. This repository is public: no
 * fixture in it will ever carry usable key material, and the value exists only
 * so a leak assertion has a distinctive needle to search for.
 */
...
describe("project-connections repository", () => {
  ...
  it("rejects a second ACTIVE connection on one project in the database, not by a prior read", async () => {
    ...
    // The refusal must come from the constraint. A generic "not implemented"
    // cannot satisfy this: the message has to name the unique index or the
    // duplicate-key violation, which only real SQL produces.
  ...
  it("yields exactly one active connection when two attach attempts race", async () => {
    ...
    // Exactly one winner, decided by the index. There is no read-then-write
    // window here for both to pass through.
  ...
  it("re-attaching the same source after a deactivate succeeds and keeps the old row", async () => {
    ...
    // The partial index only covers active rows, so the same source attaches
    // again rather than being permanently refused.
    ...
    // History survives the cutover: the deactivated row is kept, not deleted.
    // Read raw here only because no repository method lists inactive rows —
    // every SCOPING assertion in this file goes through the repository.
    const rows = await db
      .select()
  ...
  it("returns null from every mutation for a foreign org's connection and changes nothing", async () => {
    ...
    // Never a silent success: org A's own scoped read must show an untouched
    // row, not "returned null while mutating anyway".
    const repoA = createProjectConnectionsRepo(db, orgA.ctx);
    const after = await repoA.getActiveForProject(project.id);
