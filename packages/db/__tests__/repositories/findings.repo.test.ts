// findings repository (O-011) — the first analysis-side persistence, and the
// two clauses of the outcome that only real SQL can prove.
//
// Every test below runs against the real generated migrations via
// `createTestDb()`'s PGlite instance, the same harness
// `deliveries.repo.test.ts` uses. That is deliberate: both assertions here are
// about what the DATABASE holds — which columns a write stamps, and whether an
// unreported count lands as SQL `NULL` rather than as `0`. A fake repository
// would agree with itself and prove neither.
//
// Reading order, by the taxonomy dimension each block pins:
//   D2  stamp/filter symmetry — every column the read path filters by
...
//       never as an error.
//   D5  not-reported is not zero — `tokens_in`/`tokens_out` are nullable
//       because null MEANS "the SDK reported no count" (AD-5,
...
//
// NOTE (ADD v2 AD-20): `findings.signature` IS the finding's identity, stored
// beside O-006's ledger the way `deliveries.signature` and
// `dismissals.signature` already are. The repository ACCEPTS a signature and
// never mints one — `computeFindingSignature` remains the single producer, a
// layer up — so nothing here derives an identity, and the fixtures below are
...
/** A distinct signature, in the shape the column really holds — 64 lowercase
 * hex characters. Deliberately NOT `computeFindingSignature`: this suite proves
 * what the DATABASE does with the value, and re-running the real producer here
 * would couple a storage test to a tuple serialisation it is not about. */
function signatureFor(label: string): string {
  return sha256Hex(`findings.repo.test:${label}`);
}

/**
 * A persistable finding. `counts` is empty on purpose: the composition of a
 * measured count is `packages/core`'s subject and is tested there — this suite
 * is about which columns the write stamps and how an absent token count
 * lands, and a count row would prove neither.
 */
function makePersistInput(
  ...
  return {
    ...
    // One sentence per element (AD-8) — never a blob to be re-split later.
    context: [
...
describe("findings repository", () => {
  ...
  it("every column the findings read path filters by is stamped by the write path", async () => {
    ...
    // The write stamps BOTH scoping columns. If either were left unstamped,
    // every scoped read below would match zero rows and the lane would look
    ...
    // Both filters genuinely narrow — without these two, the assertions above
    // would also pass against a read that ignores its filters entirely.
  ...
  it("unreported token counts persist as not reported and are never written as zero", async () => {
    ...
    // `summaryUsageSchema`'s fields are `.optional()`, so an unmetered call
    // arrives as `undefined` — never pre-coerced to a number by the caller.
    ...
    // Named explicitly, because this is the failure being prevented: a
    // candidate the model touched but did not meter must NOT be
    // indistinguishable from one that cost nothing.
    expect(written.tokensIn).not.toBe(0);
    expect(written.tokensOut).not.toBe(0);

    // The same fact survives the round-trip through SQL, not just the
    // `RETURNING` row.
    ...
    // The contrast that gives "never zero" its meaning: a genuinely reported
    // zero is a different, storable fact — the column is not simply refusing
    // zeroes.
  ...
  it("an unrecorded surface normalisation version persists as not recorded and a genuine zero remains a distinct storable version", async () => {
    ...
    // Exactly how a candidate hands this over when no normaliser version was
    // recorded: `CandidateFinding.surfaceNormalisationVersion` is
    // `z.number().int().nullable()` (`core/src/findings/candidate.ts:93`), so
    // the absence arrives as `null` — never pre-coerced to a number by the lane.
    ...
    // Named explicitly, because this is the failure being prevented: the column
    // used to be NOT NULL and the lane wrote `0` to mean "none recorded".
    ...
    // The contrast that gives "never zero" its meaning, and the reason this is
    // not pedantry: the candidate contract is `.nullable()` and NOT
    // `.positive()`, so `0` is a version a normaliser may legitimately report.
    // It is a different, storable fact — and on a column that feeds D12
    // identity comparisons, "normaliser v0" and "we do not know" must never be
    ...
    // The pair, stated directly: the two rows are distinguishable in the
    // database, which is the entire property.
