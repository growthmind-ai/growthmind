// Wave 0 (RED) — O-009 `mcp-read-credential`, ADD §8 "Structural gates
// (source scans, non-vacuity required)", all 5 rows.
//
// `packages/db/src/admin/` is an UNSCOPED cross-tenant enumeration by
// construction (ADD D-9): it must list every organisation before any scope
// exists, because a CLI has no session to derive scope from. Its only two
// defences are the `"./admin"` subpath — which keeps a violating import to a
// single greppable line — and this file, which turns that convention into a
// gate. Modelled on `packages/db/__tests__/system/reachability.test.ts`, with
// one deliberate difference: `scripts/` is ALLOWED here, because `scripts/` is
// exactly and only where this module is meant to be reached from.
//
...
//     the D-10 seam gate: if O-008's UI has to re-implement minting, this
//     sprint got the seam wrong, and this row is what notices.
//
// Per ADD OQ-3 the matcher's positive control is a SYNTHETIC source string,
// never a fixed barrel export list — O-011 appending its own exports must not
// break this file.
...
// Every file the gate covers: `apps/`, `worker/`, and every package's own
// `src` directory. `scripts/` is deliberately absent — it is the one allowed
// caller (D-9) — and each package's `__tests__` is out of scope because the
// walk starts at its `src`.
 ...
 *
 * Resolving (3) rather than pattern-matching `/admin` keeps the gate from
 * firing on an unrelated directory that happens to be named `admin`, so a
 * future failure here is always a real violation.
 */
...
describe("admin subpath unreachability (D-9)", () => {
  ...
  it("should not be re-exported from the main barrel", () => {
    ...
    // `@growthmind/db` — a source-level check alone would miss a re-export
    // routed through another module.
    ...
    // scoped surface. One stable symbol, not a fixed list — O-011 appending
    // exports must not break this (OQ-3).
  ...
  it("should actually read the files it claims to check", () => {
    ...
    // Known files that MUST be in the scanned set — the composition root that
    // would be the most tempting place to import an unscoped enumeration
    // from, and the barrel this gate is about.
    expect(files).toContain("apps/web/app/api/mcp/route.ts");
    expect(files).toContain("packages/db/src/index.ts");

    // And `scripts/` is deliberately NOT scanned: it is the one allowed
    // caller, so a scan that covered it would fail the moment Wave 5 lands.
    expect(files.some((file) => file.startsWith("scripts/"))).toBe(false);
    expect(files.some((file) => file.includes("node_modules"))).toBe(false);
  });

  it("should notice a violating import", () => {
    // Synthetic sources, never files on disk (OQ-3) — this row proves the
    // matcher can fail, which is the only thing that makes the empty result
    // above mean anything.
    ...
    // The relative form only a file INSIDE packages/db/src can write — the
    // barrel re-export that would undo the whole boundary.
...
describe("CLI purity (D-10)", () => {
  ...
  it("should keep the CLIs free of credential logic", () => {
    for (const relativePath of CLI_FILES) {
      ...
      // Named explicitly so the red reads "the script is missing", not
      // "ENOENT" from a readFileSync deep inside a loop.
      ...
      // No minting, no hashing, no persistence, no query builder. If any of
      // these appears here, the seam is in the wrong place and O-008's UI
      // will have to re-implement it.
