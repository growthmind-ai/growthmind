...
//      persisted (edge taxonomy D5); and the output schemas carry real
...
//
// ---------------------------------------------------------------------------
// THE ONE CATCH, AND THE ONE LOG (D8)
// ---------------------------------------------------------------------------
//
// A fault in a read, in the renderer or in an output schema is caught here and
// becomes `UNAVAILABLE`. THIS IS THE ONLY PLACE A TOOL FAULT IS LOGGED. The
// layer above keeps a catch of its own for a fault that escapes IT — a
// different fault, with a different message — and the two can never both fire
// for one event, because this function does not throw. Two catches claiming one
// fault is the shape that makes a log unreadable during an incident, and
// `__tests__/mcp/failure-isolation.test.ts` asserts the count is exactly one.
//
// ---------------------------------------------------------------------------
...
// file has that could write anything is `McpReadPort`, whose three methods are
// all reads. `report_shipped`, the draft contract's one write tool, is absent
// from the descriptor list and asserted absent by name in
// `packages/shared/__tests__/mcp/tools.test.ts`.
//
// Precedent: `worker/src/tasks/delivery-tick.ts` — the task body is a plain
// function over ports, and the Graphile-aware wrapper decides nothing.
 ...
 * lookup for the one schema this file needs at module scope, and it THROWS at
 * import time if a name in the contract has no descriptor — a boot failure the
 * first call notices, rather than a `undefined.parse` on a live one.
 */
...
export async function callTool(
  ...
  try {
    ...
    // the renderer, or in an output schema is one detail-free refusal rather
 ...
 *
 * The name is the RESOLVED one rather than the string off the wire, so the
 * sentence `malformedInput` builds names the tool the contract knows about and
 * not whatever spelling arrived.
 */
...
/** One producer of the answered arm, so every success carries the name of the
 * tool that produced it and nothing else is ever added beside it. */
 ...
 * throw reaches the one catch in this file and becomes `UNAVAILABLE` with no
 * detail: a fix spec that cannot be rendered safely is one we do not serve, and
 * it is emphatically not a `NOT_FOUND`, which would tell an agent the work does
 * not exist.
 */
