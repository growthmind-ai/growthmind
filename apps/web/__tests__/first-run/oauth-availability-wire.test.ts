...
// string cannot tell "on the screen" from "in the DOM but folded away" — which
// is the ONE distinction row 2 turns on. So the markup is walked once, and what
// comes back is what a person could read or a screen reader announce: text
// outside every hidden subtree, plus the accessible name of each control.
//
...
// this reader gives the same answer for both. Wave 6 picks the widget; this
// file only says the founder cannot see what is behind it.
// ===========================================================================

/** What one render of the delivery card looks like from the outside. */
...
function readMarkup(html: string): RenderedCard {
  const words: string[] = [];
