// WHAT ONE REQUEST CAN BUY, AND WHAT WE HEAR WHEN THE TRANSPORT REFUSES IT —
// WIRE-L1…L8 (O-013, post-sprint audit).
//
// Every other file in this directory asks whether an answer is CORRECT. These
// rows ask whether it ARRIVES, and how much a caller had to be given to get it.
// Three measured holes, none of them visible to a single row of the sixty-nine
// the sprint shipped, because all three live outside the shapes those rows
// mint:
//
//   H-1  a modern `subscriptions/listen` answered by the SDK's listen router
...
//
//   M-1  nothing between the credential check and the transport bounded a
...
//
//   M-2  `createMcpHandler` was given no `onerror`, and the SDK RETURNS rather
...
// observe. THE PARTITION IS THE CLAIM — one incident, one line, and the line
// says which layer broke. `WIRE-L7` proves a transport fault reaches the third
// channel; `WIRE-L8` proves a broken READ still does not, which is the property
// `./failure-isolation.test.ts`'s `WIRE-B1` asserts from the other side and the
// one an over-eager `onerror` would have broken.
//
// ---------------------------------------------------------------------------
// BOTH LEGS, EACH WHERE IT BELONGS
// ---------------------------------------------------------------------------
//
// The size and batch gates fire in `server.ts`, in front of the transport and
// therefore in front of the leg split, so those rows are minted through the
// legacy fixture like everything else. `subscriptions/listen` exists only on
// the modern leg, so its rows are minted through `./helpers/modern-envelope.ts`
// — the file that carries the decision the fixture's header asked for.
//
// Lane prefix `mcpl`.
 ...
 *
 * GENEROUS BY THREE ORDERS OF MAGNITUDE, ON PURPOSE. The refused
 * `subscriptions/listen` answers in about a millisecond; the bug it guards
 * against does not answer at all. A tight bound would turn a slow machine into
 * a red row about nothing, and a bound of any size catches a deadlock.
 */
 ...
 *
 * The timer is cleared on the winning path so bun's runtime is not left with a
 * pending macrotask holding the suite open, and the losing promise is left to
 * settle on its own: a row that has already failed must not also hang.
 */
 ...
 * `notifications` value the transport cannot read is answered `-32602` without
 * ever reaching the listen router, and a row built on one would pass while
 * proving nothing about the hang. This shape reaches the router: before the
 * fix it hung, and after it is refused in about a millisecond.
 */
const LISTEN_PARAMS = {
...
describe("WIRE-L1 — a modern subscriptions/listen is answered in bounded time rather than held open", () => {
  test("answers within the bound, with a body, and asks the port nothing", async () => {
    ...
    // THE ROW, IN ONE LINE. Before the fix this was the sentinel — measured
    // unanswered at 45 seconds, which is nine times this bound.
  ...
  test("refuses it in band, on a 200, rather than as a transport failure", async () => {
    // THE SHAPE OF THE REFUSAL, PINNED. `maxSubscriptions: 0` answers in-band —
    // HTTP 200 carrying a JSON-RPC error — which is what lets a client tell
    // "this server does not do subscriptions" from "this server is broken".
    // Refusing is truthful here rather than a workaround: this surface declares
    // no subscription capability and emits no notifications, so there is
    // nothing a subscriber could ever be sent.
    const { deps } = spyDeps();
...
describe("WIRE-L2 — many concurrent subscriptions/listen requests all answer", () => {
  /**
   * THE MEASUREMENT THAT MADE H-1 HIGH RATHER THAN MEDIUM. One deadlocked
   * request is a bug; fifty of them, each holding a server instance and a
   * keep-alive interval, is the surface being unavailable for the price of one
   * read-only key. Ten is enough to fail in the same way and cheap enough to
   * run on every commit.
   */
...
/** `count` `tools/call` messages in one array. At 500 this is the exact body
 * that measured 500 reads and 500 frames out of one POST. */
...
/** Comfortably over the megabyte ceiling, and shaped like a real call so the
 * refusal is about the size and not about the shape. */
...
describe("WIRE-L3 — a JSON-RPC batch is refused before it can buy any work", () => {
  test("a 500-message batch is refused 400 with the single-message instruction, and the port is untouched", async () => {
    ...
    // THE HALF THAT IS THE POINT. Not "it was refused" — "it bought nothing".
    // Before the gate this line read five hundred.
    expect(spy.organizationsAsked).toEqual([]);
  });
  ...
  test("a one-message batch is refused too, so the gate is about the shape and not the size", async () => {
    ...
    // `tools/list` nor `tools/call` is ever more than one message. Refusing the
    // shape is the truthful gate; refusing at 51 and allowing 50 would be a
  ...
  test("leading whitespace does not smuggle a batch past the gate", async () => {
    // The gate reads the FIRST NON-WHITESPACE byte, which is a fact about JSON
    // rather than a heuristic. A gate that looked at byte zero would be defeated
    // by a newline.
    const { spy, deps } = spyDeps();
  ...
  test("a single message whose arguments contain an array is still served", async () => {
    // NON-VACUITY, AND THE CONFLATION THIS GATE COULD HAVE HAD. Arrays are
    // ordinary inside a message; only a message that IS an array is a batch. A
    // gate that scanned for `[` anywhere would refuse legitimate calls, which is
    // the exclusion-predicate-on-a-superset shape the taxonomy files under D10.
    const { spy, deps } = spyDeps();
...
describe("WIRE-L4 — a body bigger than this surface could ever need is refused before it is buffered", () => {
  ...
  test("an ordinary call is still served, so the ceiling refuses nothing real", async () => {
    // NON-VACUITY. A gate that refused every body would pass the row above
    // perfectly and break every client. The ceiling is about four orders of
    // magnitude above the largest legitimate message this surface has.
    const { spy, deps } = spyDeps();
  ...
  test("the refusal names neither a stack frame nor a size in bytes", async () => {
    ...
    // actionable; a five-digit byte count is arithmetic, and a stack frame is a
    // leak. Both absences asserted on the same body.
...
describe("WIRE-L5 — reading the body to gate it does not change what the transport sees", () => {
  /**
   * THE RISK THE SIZE AND BATCH GATES INTRODUCED, CLOSED. A body can only be
   * read once, so `server.ts` reads it and hands `wire.ts` a request rebuilt
   * from the same bytes. If that rebuild lost a header, a byte, or the verb,
   * every row in this directory would still pass on the shapes it mints while
   * some other shape broke — so this row asserts the round trip on a shape that
   * exercises the transport's own parser rather than ours.
   */
  test("a malformed body still reaches the transport's parser, byte for byte", async () => {
    ...
    // The transport's parse error, not ours: the rebuilt request carried the
    // broken bytes through unchanged rather than a repaired or re-encoded copy.
  ...
  test("a well-formed call answers exactly as it did before the body was ever read", async () => {
    ...
    // Byte-identical twice, which is the floor the cross-tenant proof stands on
    // and the property a stale `content-length` on a rebuilt request would have
    // been most likely to break.
...
/** The word that tells the transport's channel apart from the other two. Both
 * rows below read it, so a message reworded in `wire.ts` without a thought for
 * the partition fails here rather than during an incident. */
...
describe("WIRE-L7 — a fault the SDK reports reaches a log line of ours", () => {
  test("a transport-level refusal logs exactly once, naming the transport", async () => {
    // WITHOUT AN `onerror` THIS WAS SILENT — measured: a wire-layer failure
    // answered the caller and wrote nothing at all, because the SDK returns
    // rather than throws and `server.ts`'s catch never fires.
    ...
    try {
      ...
      // ONCE. Never zero, which is the regression this closes; never twice,
      // which is two channels claiming one event.
...
describe("WIRE-L8 — a broken read still belongs to the tool core's channel and not the transport's", () => {
   ...
   * `./failure-isolation.test.ts`'s `WIRE-B1` requires exactly one log line for
   * a broken read; this row adds WHICH line it must be. `callTool` catches its
   * own fault and returns a refusal value, so nothing throws into the SDK and
   * the transport channel must stay quiet — if it ever speaks here, one
   * incident has become two lines that disagree.
   */
  test("logs once, and the line is not the transport's", async () => {
    ...
    try {
      ...
      // The precondition: the broken read really did produce the answer this
      // row is about.
