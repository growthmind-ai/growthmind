...
//
// ===========================================================================
// O-013: THE ROWS ARE `WIRE-R1…R22`, AND TWO OF THEM CHANGE THEIR CLAIM
// ===========================================================================
//
// Twenty-one rows live here (`WIRE-R10` is the source scanner and lives in
// `./refusal-identity-guard.test.ts`). Every one keeps its claim except two,
// and both exceptions are named deviations with a reason on the record:
//
...
//     GET. The catalogue moved onto the wire protocol as `tools/list` (D-4), so
...
// {…}\n\n' }` under the pinned `responseMode: "sse"` (D-4). Pre-SDK refusals —
// the 401 and the 405, both produced by `refusalResponse` before `wire.ts` is
// called at all — arrive as `{ 4xx, "application/json;charset=utf-8", … }`.
// Both constants are named below and asserted by the rows that sit in them.
//
// THIS FILE IS NOT ONE OF THE FOUR IDENTITY SUITES, so `JSON.parse` is
// available here and is used, deliberately, to read a result's fields. That is
// not a loosening: `WIRE-R10` bans parsing in `cross-tenant.test.ts`,
// `cross-tenant-real-keys.test.ts`, `credentials.test.ts` and
// `api-key-credentials.test.ts` because parsing discards the bytes those files
// exist to compare. Nothing in this file compares a cross-tenant pair — the two
// identity rows it does carry (`WIRE-R12`, `WIRE-R22`) compare raw
// `fingerprint`s and never a parsed object.
//
// PROTOCOL ERRORS ARE READ BY RAW CONTAINMENT, NOT BY FRAME. `-32700` and
// friends are the TRANSPORT'S to emit (D-7), and the framing it wraps them in
// is its business and was not measured. The rows that assert one look for the
// serialised code in the raw body, exactly as `./wire-envelope.test.ts` does,
// so they cannot fail on a framing detail nobody pinned.
//
...
/** The two content-type bands (D-6), both measured exactly. */
...
/** A credential that resolves over a store that records every organization it
 * was asked about — for the rows whose claim is that a port was NOT reached. */
 ...
 *
 * `sseDataLine` does the unframing with string operations only; the parse is
 * this file's own, and is allowed here for the reason the header states.
 */
...
/** The `result` of an SDK-rendered answer. Throws rather than returning
 * `undefined`, so a row that reached for a result on an error frame fails with
 * a sentence instead of comparing against nothing. */
...
/** The first block of text a tool result carries — the place every refusal
 * sentence lands once it travels as a tool execution error (D-6 rule 2). */
...
/** The wire form of a protocol error code inside a serialised JSON-RPC error
 * object — matched as a substring of the RAW body, because the framing the
 * transport wraps its own errors in is its business and was not measured. */
...
describe("what the surface refuses, and how it says so", () => {
   ...
   *
   * It is achievable ONLY under D-7's Surface B, and `W0-P2d` measured exactly
   * why. With `registerTool` alone, `McpServer` resolves `params.name` against
   * its own registry FIRST and answers
   * `-32602 "Tool not_a_tool not found"` — a protocol error carrying none of our
   * tool names and none of our sentence. Under Surface B, a `tools/call`
   * override on the INNER `server.server`, registered AFTER the `registerTool`
   * loop, receives `params.name` itself and answers `resolveMcpTool`'s own
   * message.
   *
   * So a red here almost certainly means one of three things, in order of
   * likelihood: the override is missing; it is registered on the `McpServer`
   * facade rather than on `server.server`; or it is registered BEFORE the loop
   * and the loop overwrote it. Measured: `setRequestHandler` does not throw when
   * it lands correctly.
   */
  test("WIRE-R1 — an unknown tool name is refused with the names that exist, never with a server error", async () => {
    ...
    // The band first: an unknown tool is OUR refusal travelling as a tool
    // execution error on 200, never the SDK's `-32602` and never a 500.
  ...
  test("WIRE-R2 — a body that is not JSON is refused as a parse error in plain English with no stack trace", async () => {
    ...
    // THE PRECONDITION: this really is the parse refusal, and not a 406 or a
    // 500 that also happens to carry no stack frame. The two absences below are
    ...
    // The framing is the transport's (D-7) and the sentence in it is not ours,
    // but what must never ride along is: a stack frame, a file path, or the
    // engine's own exception name.
  ...
  // NON-VACUITY for the two leak scanners the row above leans on. A scanner
  // that has gone blind reports "no leak" on a body that is nothing but leak.
  ...
  test("WIRE-R3 — a tools/call with no tool name is refused rather than guessed at", async () => {
    ...
    // `params` present, `name` absent — the shape a client sends when it has
    // built the envelope correctly and forgotten what it was calling.
    const print = await fingerprint(
    ...
    // Answered rather than thrown on, and answered as a message rather than as
    // an empty body.
    expect(print.status).toBeLessThan(500);
    expect(print.body).toContain('"jsonrpc":"2.0"');

    // THE HALF THAT MATTERS: a call that named no tool cannot have touched
    // anything that reads data. Never guessed at, never defaulted to the tool a
  ...
  test("WIRE-R4 — a limit above the ceiling is refused, never quietly capped", async () => {
    // The boundary case: one more than the ceiling. A surface that CAPPED would
    // answer 200 with a result here, which is the thing this row forbids.
    ...
    // And a value far above it, chosen so the "never echoed" assertion below
    // cannot pass by coincidence: `26` could plausibly appear inside some other
    // number on the wire, `987654` could not.
    ...
    // The value the CALLER SENT never travels back. It may be somebody's id in
    // another field, and a refusal that echoes its input is a refusal that can
  ...
  /**
   * ⚠️ CLAIM INVERTED (D-4). This row used to read "a method other than GET or
   * POST"; `GET` is now refused too, because the catalogue it used to serve
   * moved onto the wire protocol as `tools/list`. The row is NOT dropped — an
   * agent that GETs must still receive a sentence saying what to send instead,
   * which is why `GET` stays an explicitly exported handler returning
   * `refusalResponse(WRONG_METHOD)` rather than being delegated to the SDK's own
   * bodiless 405.
   */
  test("WIRE-R7 — every method other than POST is refused, including GET", async () => {
    for (const method of ["GET", "DELETE"]) {
      ...
      // PRE-SDK 405 BAND: `GET` never reaches the transport at all, so the
      // framing pin cannot touch this answer.
    ...
    // The other half of "unmounted": `PUT`/`PATCH`/`DELETE`/`HEAD`/`OPTIONS`
    // are not exported by the route module at all, so Next answers them with
    // its own 405 and nothing of ours ever runs. Asserted structurally, because
    // a handler test cannot exercise Next's routing.
  ...
  test("WIRE-R8 — every refusal this surface can produce is plain English with no product vocabulary", () => {
    ...
    // The two refusals built by a function rather than frozen as a constant are
    // audited too — they are just as customer-facing.
    const messages = [
   ...
   *
   * Copied verbatim from `origin/main`. The point is not that the sentences are
   * good — it is that a reword becomes a FAILING TEST WITH THE OLD TEXT VISIBLE
   * IN THE DIFF, so the change is a decision somebody makes rather than one that
   * happens. Both carry the cross-tenant guarantee: `NOT_FOUND` is the same
   * answer for a foreign id and a nonexistent one, and `UNAUTHENTICATED` is the
   * same answer for all six credential failures.
   *
   * Frozen is asserted too, because a constant that can be mutated at runtime is
   * a constant only by convention.
   */
...
describe("an installation with nowhere to record a fix answers truthfully", () => {
  test("WIRE-R11 — list_open_fixes answers a well-formed empty list rather than crashing or inventing a row", async () => {
    ...
    // ⚠️ READ FROM `structuredContent`, AND THAT IS LOAD-BEARING (D-15). A tool
    // that advertises an output schema and answers without schema-valid
    // structured content is REJECTED by a real client that has listed first —
    // measured, `ProtocolError -32600`. The server does not enforce it, so this
    // row is where a handler-level test can see the value at all.
  ...
  test("WIRE-R12 — get_fix and get_finding answer exactly as they answer an id that does not exist", async () => {
    ...
    // The band before the identity: two answers that were both the wrong kind
    // of answer compare equal to each other all day.
...
describe("what a tool answers when there is something to answer with", () => {
  ...
  /**
   * THE BEHAVIOURAL TWIN OF `WIRE-J2`'s io-input assertion, and `WIRE-E3`
   * confirms the same thing through a real client.
   *
   * `limit` is declared `.default(...)` with no `.optional()`, so an
   * OUTPUT-side rendering of the schema would advertise it as REQUIRED and a
   * strict client would refuse the zero-argument call an agent makes first. This
   * row is the server-side half of that proof: the call is legal and the port is
   * reached exactly once.
   *
   * The request carries NO `arguments` key at all — not `arguments: {}` — which
   * is what a client that has nothing to send actually puts on the wire, and a
   * different code path from an empty object.
   */
...
describe("this surface reads and does nothing else", () => {
  /**
   * ⚠️ THE `toEqual` IS DELIBERATE AND SURVIVES THIS SPRINT UNCHANGED. D-4 adds
   * NO export to the composition root: the SDK handler is constructed inside
   * `wire.ts`, `handleMcpRequest` keeps its signature, and research OQ-6 closes
   * with "nothing new is exported". An exact-equality assertion is what makes
   * that a decision rather than a hope — a fifth export fails here, loudly, on
   * the day it is added.
   */
  ...
  /**
   * THE CATALOGUE, AFTER IT MOVED ONTO THE WIRE PROTOCOL (D-4).
   *
   * Four fields per tool, because those four are what a client's tool picker
   * and a model's decision to call actually consume: the name it invokes, the
   * title a person reads, the description a model reads, and the input schema
   * its own parser compiles. `annotations.readOnlyHint` is the fifth and it is
   * the read-only promise arriving where a client can see it — asserted here
   * rather than only on our descriptors, because a promise that never reaches
   * the wire is not a promise.
   */
  ...
  /**
   * THE ANONYMOUS ORACLE, CLOSED — AND NOW ALSO PROVING THE PARSER WAS NEVER
   * REACHED (D-6 rule 1).
   *
   * The syntactically invalid body is the load-bearing case. A surface that
   * parsed before it authenticated would answer it with a `-32700` and answer
   * the well-formed one with a 401 — two different answers, and therefore an
   * oracle an anonymous caller can use to learn whether its payload shape was
   * right. Byte-identity between the two is the proof that authentication runs
   * on the raw `Request`, in front of the envelope, exactly as `WIRE-O1`
  ...
  test("WIRE-R22 — an anonymous caller learns nothing except that it is anonymous", async () => {
    ...
    const [first] = answers;
    // PRE-SDK 401 BAND, asserted before the comparison. Six identical wrong
    // answers would satisfy the loop below without proving anything.
    ...
    // And the parser really was never reached: not one of the six carries a
    // parse error, though two of them are unparseable.
    for (const answer of answers) {
