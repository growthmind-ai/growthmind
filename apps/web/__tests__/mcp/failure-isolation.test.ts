// WHEN THE READ BREAKS — WIRE-B1 / WIRE-B2 (O-013, lane W0-T-A, D8).
//
// A tool call is a main operation with a fault line running through it: the
// read can fail. What happens then is not a detail — it is the difference
// between a coding agent that reports "the server is having a problem, try
// later" and one that pastes a stack trace naming our files into a customer's
// terminal, or worse, hangs because a rejection escaped the handler and no
// answer was ever framed.
//
// Two rows, one fake:
//
...
//
// `throwingReadPort()` lives in the fixture rather than here because three §6
// rows need the same fake — these two and `WIRE-S5` — and three hand-rolled
// copies would be three chances to throw a subtly different thing and prove
// three different properties. Its message names the fixture, so a leaked frame
// is unmistakable in the scan below.
//
// ---------------------------------------------------------------------------
// WHY BOTH ROWS ASSERT THE PORT WAS ACTUALLY REACHED
// ---------------------------------------------------------------------------
//
// Both claims are about what happens AFTER a read throws, and both are
// satisfied trivially by a surface that never reads at all — which is exactly
// what this surface does today, because a JSON-RPC body reaches the Wave 0
// route as an object with no `tool` key and comes back as a 400 before any port
// is touched. A row that only asserted "no unhandled rejection" would be green
// now, green through wave 8, and green forever after somebody accidentally
// disconnected the read port.
//
// So each row establishes first that the answer is the one the broken read
// produced — `UNAVAILABLE.message`, the sentence that only exists on that path
...
describe("WIRE-B1 — a read that throws becomes a detail-free answer with the fault logged", () => {
  test("answers with the unavailable sentence, carries no stack frame or file path, and logs once", async () => {
    ...
    try {
      ...
      // THE PRECONDITION. Everything after this line is an absence, and an
      // absence asserted about a response the broken read never produced is
      // worth nothing.
      ...
      // Once, not never and not twice. Never means a real outage is invisible
      // to us; twice means two catches are both claiming the same fault, which
      // is the shape that makes a log unreadable during an incident.
...
describe("WIRE-B2 — no unhandled rejection escapes the mounted handler", () => {
   ...
   * process-level warning nobody reads. A rejection that escapes a
   * request-scoped handler in a serverless runtime can take the whole
   * invocation down, and the caller sees a socket close rather than a refusal.
   *
   * The watcher flushes a macrotask before reading its list, because the
   * runtime reports an unhandled rejection after the microtask queue drains —
   * a watcher that read immediately would report "clean" every time.
   */
  test("a rejecting port still comes back as a Response, with nothing left unhandled", async () => {
    ...
    // The precondition again: the read really did run and really did throw, so
    // there was something that COULD have escaped.
