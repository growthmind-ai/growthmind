...
//
// ===========================================================================
// O-013: THE ROWS ARE `WIRE-C1…C6`, AND ALL SIX SIT IN THE PRE-SDK 401 BAND
// ===========================================================================
//
// The surface moved to JSON-RPC over a real MCP transport, and NOT ONE ROW IN
// THIS FILE MOVED WITH IT. That is the finding worth writing down rather than a
// convenience: `server.ts` authenticates on the raw `Request` — before the
// Origin gate, before the Content-Type gate, before the body is read, before
// `wire.ts` is called at all — so every answer below is produced by
// `refusalResponse` with the SDK nowhere in the call stack (D-6 rule 1).
//
...
//     `WIRE-G6(b)` asserts from the other side.
//
// EVERY ROW NOW ASSERTS ITS BAND BEFORE IT ASSERTS ITS IDENTITY. Two answers
// that were both the wrong kind of answer compare equal to each other all day;
// the band assertion is what stops an identity row passing on a pair of 406s.
// The rows below therefore pin status AND content type first, and only then
// compare bytes.
//
// ⚠️ `WIRE-C6` IS UNTOUCHED, BYTE FOR BYTE — no id prefix, no band assertion,
// not a character. It is the proof PR #16's credential path is reused verbatim
// by this sprint (ADD §4 row 33), and an edit to it would destroy the thing it
...
/**
 * The content type of every answer OUR `refusalResponse` produced, before the
 * SDK was ever in the call stack (D-6 rule 1). Measured exactly, charset suffix
 * and all — `Response.json` writes it and nothing downstream rewrites it.
 *
 * The sibling band is `text/event-stream`, and no row in this file is in it. If
 * one of these rows ever reports that value, the 401 has moved BEHIND the
 * transport and the largest identity set in the sprint has lost its immunity to
 * both the framing pin and the `Accept` 406.
 */
...
describe("the read surface admits no ingest credential, and says nothing about why", () => {
  ...
  test("WIRE-C2 — refuses a genuine unrevoked standard ingest key exactly as it refuses no key at all", async () => {
    ...
    // transport's 406 or a tool error that happened to match — an identity
    // comparison between two wrong answers is no proof at all.
  ...
  test("WIRE-C4 — a missing, malformed, unknown, revoked and wrong-family credential all answer identically", async () => {
    ...
    const [first] = answers;
    // The band, before the five-way comparison — five identical wrong answers
