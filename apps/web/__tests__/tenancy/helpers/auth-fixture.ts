...
// decisions). Built once so the parallel apps/web integration suites (signup-org,
// session-context, member-addition, cross-tenant, redirects) share one working harness
// instead of each reinventing it. Extracted and generalized from the proven, passing
// `auth-hooks.spike.test.ts`.
//
// This file constructs its own directly-built Better Auth instance, never the
// `apps/web` lazy singleton in `lib/auth.ts` (a later wave owns that file; it has
// existing PostHog hooks this fixture must not disturb).
...
/**
 * Callbacks a suite can inject into the two `databaseHooks` entries wires in production
 * (`apps/web/lib/auth.ts`, a later wave): `user.create.after` (the org auto-creation
 * trigger) and `session.create.before` (`activeOrganizationId` stamping). Omitting a
 * hook is a first-class fixture state, not an oversight. It simulates that hook not
 * firing, which is exactly the state the self-heal-path suite needs to construct.
 */
...
/**
 * Builds a Better Auth instance over a caller-supplied PGlite db (typically from
 * `createTestDb`). Use this directly when a suite needs several repositories/auth
 * instances sharing one db. E.g. the cross-tenant fixture, which signs up users into
 * two different organizations against one db.
 */
...
/**
 * One-liner setup for suites that only need a single db + single auth instance: boots a
 * fresh PGlite db, builds Better Auth over it, and hands back both plus a `close`.
 * Equivalent to `createTestDb` followed by `createTestAuth` for suites that don't
 * need to share the db across multiple auth instances.
 */
...
export interface SignedUpTestUser {
   ...
   * auth-hooks.spike.test.ts). A suite that needs to simulate an authenticated call has
   * the raw token here; this fixture does not prove a header/cookie encoding for it.
   * Derive that yourself if a suite needs a real authenticated `fetch`/`getSession`
   * round-trip.
   */
  token: string | null;
}
...
/**
 * Wraps `auth.api.signUpEmail`, the real product path / require. Returns a minimal,
 * stable shape (not Better Auth's raw response) so suites assert against a fixed
 * contract regardless of plugin-added fields.
 */
/**
 * Structural minimum these helpers need, rather than the nominal `TestAuth`. Suites may
 * pass either the fixture's own instance or the real production one built through
 * `buildAuth`'s `{ db, secret, baseURL }` test seam. Those are structurally identical
 * but distinct generic instantiations, so a nominal parameter type would reject the
 * production instance and push suites back toward asserting against a replica of the
 * app instead of the app.
 */
...
// Member addition (Better Auth's real server-side organization API,
// never a raw `member`-row insert)
...
/**
 * Wraps `auth.api.addMember` verbatim, deliberately no try/catch. A duplicate add
 * throws `APIError` (`USER_IS_ALREADY_A_MEMBER_OF_THIS_ORGANIZATION`), per
 * auth-hooks.spike.test.ts; whether our own service edge swallows that as idempotent
 * success is an implementation-wave decision, so this helper surfaces the throw for the
 * member-addition suite to assert on directly rather than hiding it here.
 */
...
// auth-hooks.spike.test.ts's header comment). Filtering in-memory here mirrors that
// precedent instead of adding one just for test helpers.
...
/**
 * Reads back the persisted organization + membership row for `(userId, organizationId)`
 * and assembles a real `TenantContext`. The only input `createXRepo(db, ctx)` factories
 * in `@growthmind/db` accept. Throws if the membership doesn't exist: unlike
 * `deriveTenantContext`'s "no memberships yet" case, a caller here supplies both ids
 * explicitly, so a missing row means the test's own setup is wrong, not a legitimate
 * empty state to model.
 */
...
/**
 * Creates an organization + owner membership directly, bypassing `ensureOrganization`
 * (still an unimplemented stub at Wave 0). This is fixture setup only. Production org
 * auto-creation is `ensureOrganization` (`apps/web/lib/ensure-organization.ts`), not
 * this helper. A suite testing `ensureOrganization` itself (or the self-heal path) must
 * call the real function, not this one.
 */
