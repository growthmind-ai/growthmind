...
// tasks/tenancy-app-shell/add.md, D-B/D-D/D-E; PRD FR-8; edge-taxonomy
// D1/D7). O-002's definition of done literally names this test — "a
// cross-tenant access test proves org A cannot read or mutate org B rows" —
// so it may never be weakened, skipped, or downgraded to fakes. It runs
// against REAL SQL via PGlite (`createTestDb()`, wired through
// `./helpers/auth-fixture.ts`) because a fake repository proves nothing
// about SQL-level tenant scoping.
//
// §9 fixture shape: two users in org A (owner A1, non-owner teammate A2)
// and one user in org B. Memberships are built through the real product
// path — `signUpTestUser` (auth.api.signUpEmail) and `addTestMember`
// (auth.api.addMember) — never a raw `member`-row seed (FR-9).
// `ensureOrganization` is still an unimplemented D-C stub, so organizations
// are bootstrapped via `createTestOrganization` (fixture-only, per its own
// doc comment).
//
// Every repository factory in `@growthmind/db` is currently a typed stub
// whose method bodies throw `new Error("not implemented")` (D-B m0
// scaffold). Each test below seeds through the real repository surface
// (`.create`/`.mint`) before asserting the cross-tenant contract, so every
// test in this file fails NOW at that first stub call, always for the
// reason "not implemented" — never a compile error, never a fixture error
// (unique-constraint violation, missing row). Once a later wave fills in
// the repository bodies, the seed calls succeed and the assertions below
// are what prove D-B/D7 for real.
...
interface CrossTenantFixture {
  ...
  /** A second, non-owner member of org A — added via the real Better Auth
   * `addMember` path (FR-9), never a raw `member` row. */
  teammateA2Ctx: TenantContext;
  /** The sole user of org B — the "other org" every test's foreign-id
   * probes are launched from. */
  userBCtx: TenantContext;
}

/**
 * Builds the §9 fixture shape fresh for one test: signs up three real users
 * over the shared Better Auth + PGlite instance, bootstraps two
 * organizations (`createTestOrganization`, since `ensureOrganization` is
 * still a Wave 0 stub), adds the org A teammate through `addTestMember`,
 * and assembles a `TenantContext` for each of the three (userId, org)
 * pairs via `buildTestTenantContext` — the same construction path
 * `getTenantContext()` will use in production.
 *
 * `testId` must be unique per call so every seeded email stays globally
 * unique within this file (a sibling suite lost time to
 * `user_email_unique` violations from reused emails).
 */
...
describe("cross-tenant isolation (FR-8, D7) — the sprint's headline proof", () => {
  ...
  it("org B's user cannot read org A's projects or write-key metadata — empty or null, never data", async () => {
    ...
    // FR-8a: never org A's data — null/empty, not a rejection that could
    // hide a partial leak.
  ...
  it("org B's user's mutations of org A's rows affect 0 rows — never silent success", async () => {
    ...
    // No silent success: re-read through org A's OWN repo and prove the
    // name is genuinely untouched.
    ...
    // 3) Organization rename. D-B: `OrganizationsRepo.rename` takes no
    // organization-id parameter at all — it is structurally confined to the
    // constructing context's own org. Calling it as org B can therefore
    // only ever rename org B, never org A; we assert both halves: the call
    // returns B's own (renamed) row, and org A's row is untouched when
  ...
  it("org A's non-owner teammate CAN read org A's projects, write-key metadata, and org name", async () => {
    ...
    // Sanity on the fixture itself: A2 must genuinely be a non-owner —
    // this test is worthless if it accidentally asserts the owner cell.
    ...
    // D1: the teammate cell asserted, not assumed — A2 must see exactly
    // what the owner sees, not a narrowed view.
  ...
  it("a client-supplied foreign org or project id never widens access through any repository or service path", async () => {
    ...
    // findById(id) with a foreign project id.
    expect(await projectsRepoB.findById(projectA.id)).toBeNull();

    // rename(id, ...) with a foreign project id.
    expect(await projectsRepoB.rename(projectA.id, "Widened Access")).toBeNull();

    // revoke(id) with a foreign write-key id.
    expect(await writeKeysRepoB.revoke(mintedA.key.id)).toBeNull();

    // mint({ projectId }) with a foreign project id — the client-supplied
    // id must not widen B's mint into A's project; it must be rejected
    // outright, not silently minted against the wrong org.
