// EVERY FIRST-RUN ROUTE, DECLARED ONCE (AD-16, AD-16a, AD-17, AD-18).
//
// THE PROSE BELOW CARRIES NO ROUTE COUNT, DELIBERATELY. It used to say "the
// eight" in eight comments and a constant's name, and a sprint that added five
// routes made every one of them wrong without failing anything — the same rot
// `packages/shared/src/onboarding/route-schemas.ts` and `apps/web/lib/first-run/
// deps.ts` grew and had to be cleaned out of. A number nothing derives is a
// number the next person to add a route will not update either, so where a
// count matters it is read off `FIRST_RUN_ROUTES` and never typed.
//
// ###########################################################################
...
// # What lives here is the ROUTE surface: every route module under
// # `app/api/first-run/`, their deps seam, and the strictness prober AD-16a
// # requires.
// ###########################################################################
//
// ---------------------------------------------------------------------------
...
// as originally written is a non-strict schema on EVERY route, a 200 for a
// client-supplied `projectId`, and every enumeration row green.
//
...
// ---------------------------------------------------------------------------
// AD-16 — every route on this surface, and nothing else may be under that tree
// ---------------------------------------------------------------------------

/**
 * One first-run route, as AD-16's own table declares it (lines 477-486).
 *
 * `declaredKeys` is the EXACT input-schema key set from that table. A minority
 * of routes take fields; MOST take none, and for those `declaredKeys` is `[]` —
 * which under AD-16a still means `z.strictObject({})`, NOT an absent schema
 * and NOT `z.object({})`. The no-input routes are precisely the ones where a
...
export interface FirstRunRouteDescriptor {
   ...
   * adds one unknown key to. For a route whose declared input is none this is
   * `{}`, which is the ordinary body of a request with nothing to say.
   */
...
/**
 * AD-16's table, transcribed, plus the routes that deleted the hunting from
 * first run. The ORDER walks the surface top-down — status, analytics, Slack,
 * then the lifecycle pair — so a new route has an obvious place to go and a
 * missing one is visible by its neighbours rather than by counting.
 *
 * A route added under `app/api/first-run/` without an entry here fails
 ...
 * the MOUNTED verb with a PGlite handle in `getDb()`'s stash. AD-16's routes
 * need the same seam for the same reason, plus one this surface has and
 ...
 *
 * ONE NAME ACROSS EVERY ROUTE is the point: AD-16 says "every handler's
 * preamble is identical and is the whole tenancy story", and a uniform entry
 * point is what lets the §9 rows LOOP over `FIRST_RUN_ROUTES` rather than
 * hand-listing them — which is what makes them catch the NEXT route somebody
 * adds, without anybody editing a row to include it. Wave 6b may
 ...
 * constructed with `z.strictObject()` (AD-16a). For a route whose declared
 * input is none that is `z.strictObject({})` — an EMPTY STRICT object, which
 * still refuses
