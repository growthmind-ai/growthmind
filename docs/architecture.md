...
### · The pipeline is a cost funnel, and the billing meter is one of its tier boundaries
...
### · The finding primitive is a signature, not a sentence
...
### · The model proposes; deterministic evidence predicates dispose
...
### · Backpressure is a scheduler, not a filter
...
### · Growthmind supplies memory and criteria; the customer's agent supplies reasoning and typing
...
### · Everything Growthmind stores is derived, retained on a window, and exportable
...
### · Source code is processed, never stored
...
Resolved by: derived read model, retention window, adapter-first, export
...
- Derives **surfaces**, the screen or flow node that experiments lock on and that
  ...
  taxonomy diff, and the signatures survive refactors because of it.
...
Both tiers' output passes the evidence gate before it can become a finding.
...
. The only component permitted to post. Holds the token bucket, the one-open-
...
The spine of, with the numbers that make the economics work. Illustrative ratios
...
**Judgement** (`finding_signatures` _(the ledger) see )_, `findings`,
...
, the fix is not a chat endpoint but a read: return what Growthmind remembers
...
| 1   | Read-only repo access suffices                     | ,; no write tool exists in the MCP surface                     |
...
| 5   | Nothing uninspectable                              | Append-only audit log; the app is the record                   |
...
| 6   | Never assert unprovable causation                  | Evidence gate                                                  |
| 6   | Impact and action always attached                  | Expected-value function; spec renderer                         |
| 6   | Bug vs design vs changed mind                      | Finding classes with per-class proof predicates                |
| 6   | Rank by expected value                             | `packages/core/findings`, pure and tested                      |
| 6   | Stable deterministic ID                            | Signature ledger + surface-id ancestry (§4.4)                  |
| 7   | Backpressure, one at a time                        | Delivery scheduler's open-finding lock                         |
...
| 11  | Never the event source of truth                    | Derived read model + retention window                          |
...
| 12  | Never a model over every event                     | T0–T2 gate every model call                                    |
...
5. **Is the weekly finding ceiling per project, or also capped per org?** scopes
...
6. **Retention window for raw events and recordings.** That decision requires one; its length
